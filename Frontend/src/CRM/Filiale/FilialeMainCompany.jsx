      ...
      <div className={`w-full bg-card grid grid-cols-[auto_1fr] ${companyId ? "" : "border"} rounded-lg px-8 p-6 shrink-0 z-10`}>
        ...
        <div className="relative justify-end w-full gap-2 xxl:gap-4 flex items-center">
          ...
          <div className="max-w-md relative w-full">
            ...
            <Input placeholder="Caută filială..." value={searchName} onChange={(e) => setSearchName(e.target.value)} className="pl-10" />
          </div>

          <Separator orientation="vertical" className="h-full" />
          <span className="text-muted-foreground whitespace-nowrap">Filiale: {data?.total || filiale.length || 0}</span>
        </div>
      </div>
        ...
        !isFetching && (
          <div className="flex w-full h-full justify-center items-center flex-col gap-4">
            <span className="text-2xl text-muted-foreground italic">
              {searchNameDebounced.trim() === "" ? "Nu există nicio filială adăugată." : `Niciun rezultat pentru: "${searchNameDebounced}"`}
            </span>
          </div>
        )
      )}
