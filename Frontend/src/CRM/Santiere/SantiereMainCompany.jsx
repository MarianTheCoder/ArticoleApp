...
import { faMagnifyingGlass, faPlus, faColumns, faBuilding, faList, faGrip } from "@fortawesome/free-solid-svg-icons";
...
export default function SantiereMainCompany({ companyId = null, filialaId = null }) {
  ...
  return (
    <div className="h-full w-full relative flex gap-4 flex-col items-center overflow-hidden">
      {/* --- HEADER CONTROLS --- */}
      <div className={`w-full bg-card ${companyId ? "" : "border"}  grid grid-cols-[auto_1fr] rounded-lg px-8 p-6 shrink-0 z-10`}>
        <SantiereAddDialog
          filialaId={filialaId}
          companyId={companyId}
          open={open}
        ...
        <div className="relative justify-end w-full gap-2 xxl:gap-4 flex items-center">
          <div>
            <Button variant="outline" onClick={() => setIsCardView(false)} className={`gap-2 rounded-r-none ${!isCardView ? "bg-accent text-accent-foreground" : "text-muted-foreground"}`}>
              <FontAwesomeIcon icon={faList} />
            </Button>

            <Button variant="outline" onClick={() => setIsCardView(true)} className={`gap-2 rounded-l-none -ml-px ${isCardView ? "bg-accent text-accent-foreground" : "text-muted-foreground"}`}>
          ...
          <div className="max-w-md relative w-full">
            <FontAwesomeIcon icon={faMagnifyingGlass} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <Input placeholder="Caută șantier..." value={searchName} onChange={(e) => setSearchName(e.target.value)} className="pl-10" />
          </div>

          <Separator orientation="vertical" className="h-full" />
          <span className="text-muted-foreground whitespace-nowrap">Șantiere: {data?.total || santiere.length || 0}</span>
        </div>
      </div>
      ...
      {santiere.length > 0 ? (
        <div className={`p-5 ${companyId ? "pt-0" : "border"}  h-full w-full bg-card rounded-lg overflow-hidden relative`}>
        ...
        !isFetching && (
          <div className="flex w-full h-full justify-center items-center flex-col gap-4">
            <span className="text-2xl text-muted-foreground italic">
              {searchNameDebounced.trim() === "" ? "Nu există nici un șantier adăugat." : `Niciun rezultat pentru: "${searchNameDebounced}"`}
            </span>
          </div>
        )
      )}
