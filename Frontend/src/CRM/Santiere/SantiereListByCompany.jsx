    ...
    return (
      <ContextMenu>
        ...
        <ContextMenuContent>
          <ContextMenuItem
            className="gap-3"
            disabled={!santier.adresa}
            onClick={() => navigator.clipboard.writeText(santier.adresa)}
          >
            <FontAwesomeIcon icon={faMapLocationDot} /> Copiază Adresa
          </ContextMenuItem>
          <ContextMenuItem
            className="gap-3"
            disabled={!santier.latitudine || !santier.longitudine}
            onClick={() => navigator.clipboard.writeText(`${santier.latitudine}, ${santier.longitudine}`)}
          >
            <FontAwesomeIcon icon={faLocationDot} /> Copiază Coordonatele
          </ContextMenuItem>
          <ContextMenuSeparator />
          <ContextMenuItem
            className="gap-3 text-low hover:text-low focus:text-low"
            onClick={() => props.context.handleClickEdit(santier)}
          >
            <FontAwesomeIcon icon={faPenToSquare} /> Editează
          </ContextMenuItem>
          <ContextMenuItem
            className="gap-3 text-destructive focus:text-destructive hover:text-destructive"
            onClick={() => props.context.handleDeleteClick({ nume: santier.nume, id: santier.id })}
          >
          ...
          >
            {rowVirtualizer.getVirtualItems().map((virtualRow) => {
              ...
              return (
                <div
                  key={virtualRow.key}
                  className={`grid gap-4 absolute top-0 left-0 w-full p-2   ${
                    !companyId
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  ...
                                  <DropdownMenuItem
                                    ...
                                    onClick={() => handleDeleteClick({ nume: santier.nume, id: santier.id })}
                                  >
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {santier.activ ? (
                                <div className="flex items-center gap-1.5">
                                  <Badge className="text-low hover:bg-transparent bg-transparent border-low shadow-none">
                                    Activ
                                  </Badge>
                                </div>
                              ) : (
                            ...
                            <div className="flex items-center gap-2 justify-between text-base">
                              ...
                              {santier.filiala_nume ? (
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() =>
                                    navigate(`/CRM/Filiale/View/${santier.companie_id}/${santier.filiala_id}`)
                                  }
                                  className="text-base font-medium border-border bg-background hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors"
                                >
                                  {santier.filiala_nume}
                    ...
                    <div className="flex items-center pl-4 justify-start gap-4">
                      ...
                      <div className="flex flex-col items-start">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-foreground text-base cursor-text">{santier.nume}</span>
                        </div>
                      </div>
                    </div>
                  </TableCell>
