...
function SantiereColumn({ label, flag, santiere, assignedIds, search, onToggle }) {
  const filtered = santiere.filter((s) => s.nume.toLowerCase().includes(search.toLowerCase()));
      ...
      <div className="overflow-y-auto border rounded-md divide-y max-h-[52vh]">
        {filtered.length === 0 ? (
          ...
          filtered.map((santier) => (
            <label key={santier.id} className={`flex items-center ${assignedIds.has(santier.id) ? "bg-primary/10" : ""} gap-3 p-3 cursor-pointer hover:bg-muted/50 transition-colors`}>
              <Checkbox className="w-6 h-6" checked={assignedIds.has(santier.id)} onCheckedChange={() => onToggle(santier.id)} />
              {santier.culoare_hex && <span className="w-4 h-4 rounded-full shrink-0 border border-black/10" style={{ backgroundColor: santier.culoare_hex }} />}
...
export default function UtilizatoriAtribuiriDialog({ open, setOpen, onSaveAtribuiri, buttonStyle = <div className="hidden" /> }) {
  ...
  useEffect(() => {
    if (open && data) {
      const initialIds = new Set(assignments.filter((a) => a.user_id == open).map((a) => a.santier_id));
    ...
    <Dialog open={!!open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-[56rem] max-h-[80vh] flex flex-col overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>Selectează șantierele active la care este atribuit acest utilizator.</DialogDescription>
        </DialogHeader>

        <div className="relative">
          <FontAwesomeIcon icon={faSearch} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-base pointer-events-none" />
          <Input className="pl-9" placeholder="Caută șantier..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>

        {isLoading ? (
          <div className="flex-1 flex items-center justify-center text-base text-muted-foreground">Se încarcă...</div>
        ) : (
