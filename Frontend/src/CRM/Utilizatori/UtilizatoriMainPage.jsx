...
import { faMagnifyingGlass, faColumns, faList, faGrip, faPlus, faEye } from "@fortawesome/free-solid-svg-icons";
...
export default function UtilizatoriMainPage() {
  ...
  const submitCont = async (finalDraft) => {
    ...
    try {
      ...
      const fields = ["email", "name", "specializare", "password", "telephone", "telephone_1", "data_nastere", "permissions", "activ", "companie_interna_id", "permissions_template_id"];
    ...
    <div className="h-full w-full flex justify-center overflow-hidden items-center">
      <div className="w-[95%] h-[95%] flex flex-col p-4 gap-4 overflow-hidden bg-background relative rounded-lg">
        <div className="h-full w-full relative flex gap-4 flex-col items-center overflow-hidden">
          {/* --- HEADER CONTROLS --- */}
          <div className="w-full bg-card grid grid-cols-[auto_1fr] rounded-lg border px-8 p-6 shrink-0 z-10">
            <div className="flex gap-4">
              ...
              />
              <UtilizatoriAtribuiriDialog open={atribuiriOpen} setOpen={setAtribuiriOpen} onSaveAtribuiri={handleSaveAtribuiri} buttonStyle={<span></span>} />
            ...
            <div className="relative justify-end w-full gap-4 flex items-center">
              ...
              <span className="text-muted-foreground whitespace-nowrap">Utilizatori: {data?.total ?? conturi.length}</span>
            </div>
          </div>
          ...
          {/* --- CONTENT --- */}
          <div className="p-5 h-full w-full bg-card border rounded-lg overflow-hidden relative">
            {conturi.length > 0 ? (
              <UtilizatoriList
                conturi={conturi}
              ...
              !isFetching && <div className="h-full flex items-center justify-center text-muted-foreground italic text-xl">Nu s-au găsit conturi.</div>
            )}
          </div>
        </div>
      </div>
