 ...
 */
export default function TemplateEditorDialog({ open, onOpenChange, mode = "create", initialTemplate = null, companiiInterneOptions = [], onSave, onDelete }) {
  ...
  // ── helpers ──────────────────────────────────────────────────────────────
  const setPermModule = (key, val) => setPerms((p) => ({ ...p, permisiuni: { ...p.permisiuni, [key]: val } }));
      ...
      <DialogContent className="sm:max-w-[56rem] max-h-[90vh] flex flex-col overflow-hidden p-0">
        <DialogHeader className="px-6 pt-6 pb-3 shrink-0 border-b">
          <div className="flex items-center gap-2">
            ...
            <DialogTitle>{isEdit ? `Editează template — ${initialTemplate?.nume_rol || ""}` : "Template nou"}</DialogTitle>
          </div>
          <DialogDescription>{isEdit ? "Modificările se aplică tuturor utilizatorilor cu acest template." : "Creează un set de permisiuni reutilizabil."}</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6 min-h-0">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label>
                Nume rol <span className="text-destructive">*</span>
              </Label>
          ...
          <div className="flex items-center justify-between rounded-lg border p-3 bg-destructive/5 border-destructive/20">
            ...
            <Switch checked={perms.superAdmin} onCheckedChange={(v) => setPerms((p) => ({ ...p, superAdmin: v }))} />
          </div>
                ...
                {AVAILABLE_LIMBI.map((l) => (
                  <label
                    ...
                    className={[
                      ...
                      perms.limbi?.includes(l) ? "border-primary bg-primary/10 text-primary font-medium" : "border-border text-muted-foreground",
                    ].join(" ")}
                  >
                    <Checkbox
                      className="w-5 h-5"
          ...
          <div className="rounded-lg border overflow-hidden">
            ...
            <div className="grid grid-cols-[1fr_repeat(4,2rem)] items-center gap-2 px-3 py-2 bg-muted/60 border-b">
              <div className="flex items-center gap-3">
                ...
                {/* Select all toggle */}
                <button type="button" onClick={toggleAll} className="text-sm text-muted-foreground hover:text-foreground underline underline-offset-2 transition">
            ...
            <div className="divide-y">
              {PERMISSION_MODULES.map(({ key, label }) => (
                <PermRow key={key} label={label} value={perms.permisiuni?.[key] ?? ""} onChange={(val) => setPermModule(key, val)} />
              ))}
            </div>
          </div>
          ...
          <div className="flex w-full items-center justify-between">
            ...
            {isEdit ? (
              <Button
                ...
                variant="destructive"
                onClick={() => {
                  onDelete?.({ name: initialTemplate.nume_rol, id: initialTemplate.id });
                }}
              >
