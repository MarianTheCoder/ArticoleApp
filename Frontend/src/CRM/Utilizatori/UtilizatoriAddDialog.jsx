...
import { faUndo, faUser, faXmark, faEye, faEyeSlash, faShield, faPlus, faLock, faPen } from "@fortawesome/free-solid-svg-icons";
      ...
      {PERM_FLAGS.map(({ flag, color }) => (
        <button
          ...
          className={[
            ...
            flags.has(flag) ? `${color} border-current scale-105 shadow-sm` : "border-border text-muted-foreground/40 hover:border-muted-foreground/50",
...
export default function UtilizatoriAddDialog({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!localDraft.email?.trim()) {
      toast.warning("Email-ul este obligatoriu.");
      return;
    }
    if (!localDraft.companie_interna_id) {
      toast.warning("Compania internă este obligatorie.");
      return;
    }
    if (!localDraft.id && (!localDraft.password?.trim() || localDraft.password.trim().length < 6)) {
      toast.warning("Parola este obligatorie pentru utilizatori noi și trebuie să aibă minim 6 caractere.");
      return;
    }
  ...
  const handleTemplateSave = async ({ id, nume_rol, descriere, json_permisiuni }) => {
    if (!nume_rol.trim()) {
      toast.warning("Numele rolului este obligatoriu.");
      return;
    }
    try {
  ...
  return (
    <>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-8 min-h-0">
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-[auto_1fr] gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <PhoneInputCustom fixedNumber={fixedNumbers.telephone} value={localDraft.telephone} defaultCountry="RO" onChange={(v) => set("telephone", v)} />
                    </div>
                    <div className="space-y-1">
                      <Label>Telefon personal</Label>
                      <PhoneInputCustom fixedNumber={fixedNumbers.telephone_1} value={localDraft.telephone_1} defaultCountry="RO" onChange={(v) => set("telephone_1", v)} />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    <Label>
                      Companie principală <span className="text-destructive">*</span>
                    </Label>
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 h-10 px-3 rounded-md border border-input">
                      ...
                      <span className={`text-sm font-medium ${localDraft.activ ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}>{localDraft.activ ? "Activ" : "Inactiv"}</span>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-4">
                ...
                <div className="flex items-center justify-between rounded-lg border p-3 bg-destructive/5 border-destructive/20">
                  ...
                  <Switch checked={localPerms.superAdmin} onCheckedChange={(v) => setLocalPerms((p) => ({ ...p, superAdmin: v }))} disabled={isTemplate} />
                </div>
                  ...
                  <div className="rounded-lg border p-3 space-y-2">
                    ...
                    <div className="flex flex-col gap-1 max-h-24 overflow-y-auto">
                      {companiiInterneOptions.map((f) => (
                        <label
                          key={f.id}
                          className={`flex items-center gap-2 text-sm cursor-pointer rounded px-1 py-0.5 hover:bg-muted transition ${isTemplate ? "opacity-60 cursor-not-allowed" : ""}`}
                        >
                          <Checkbox
                          ...
                          />
                          <span className="px-2 py-0.5 rounded text-sm font-medium" style={{ backgroundColor: f.culoare_hex, color: getContrastColor(f.culoare_hex) }}>
                            {f.nume}
                          </span>
                        </label>
                      ))}
                      {companiiInterneOptions.length === 0 && <span className="text-sm text-muted-foreground italic">Nicio companie disponibilă.</span>}
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-lg border overflow-hidden">
                  <div className="grid grid-cols-[1fr_repeat(4,2rem)] items-center gap-2 px-3 py-2 bg-muted/50 border-b">
                    ...
                    {PERM_FLAGS.map(({ flag, label, color }) => (
                      <span key={flag} title={label} className={`w-8 h-7 flex items-center justify-center rounded border text-sm font-bold ${color}`}>
                        {flag.toUpperCase()}
                      </span>
                    ))}
                  </div>
                  <div className="divide-y">
                    {PERMISSION_MODULES.map(({ key, label }) => (
                      <PermRow key={key} label={label} value={localPerms.permisiuni?.[key] ?? ""} onChange={(val) => setPermModule(key, val)} locked={isTemplate} />
                    ))}
                  </div>
                </div>
      ...
      <TemplateEditorDialog
        ...
        initialTemplate={
          templateEditorMode === "edit" ? (selectedRole ?? null) : { json_permisiuni: localPerms } // seed "create" with current perms
        }
