...
export default function CompaniiInterneAddDialog({
  ...
  const displayLogo = draft.logo_preview || (!draft.delete_logo && draft.logo_url ? `${photoAPI}/${draft.logo_url}` : null);
      ...
      <DialogContent className="sm:max-w-[36rem]">
        <form onSubmit={handleSubmit}>
          <DialogHeader className="mb-6">
            ...
            <DialogDescription>Completează detaliile companiei tale interne.</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col items-center gap-5">
            ...
            >
              {displayLogo ? (
                <>
                  <img src={displayLogo} alt="Logo" className="absolute inset-0 w-full h-full object-contain" />
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
            ...
            <div className="flex flex-col w-3/4 gap-3 flex-1">
              <div className="grid gap-1.5">
                ...
                <Input id="nume" value={draft.nume} onChange={(e) => setField("nume", e.target.value)} placeholder="Ex: Compania Mea SRL" />
              </div>

              <div className="grid gap-1.5">
                <Label htmlFor="culoare">Culoare Brand</Label>
                <div className="flex gap-2 items-center">
                  <Input id="culoare" type="color" className="w-10 h-10 p-1 cursor-pointer shrink-0" value={draft.culoare_hex || "#3b82f6"} onChange={(e) => setField("culoare_hex", e.target.value)} />
                  <Input value={draft.culoare_hex || "#3b82f6"} onChange={(e) => setField("culoare_hex", e.target.value)} placeholder="#3b82f6" className="uppercase " />
                </div>
              </div>
            </div>
          </div>
