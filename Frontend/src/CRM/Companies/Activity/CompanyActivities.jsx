...
function UserAvatarCard({ photo, altText, sizeClass = "h-12 w-12", iconClass = "text-base" }) {
  return (
    <ImagePreviewTooltip
...
function MentionBadge({ mention }) {
  return (
    <Badge className="text-sm font-semibold flex items-center gap-1.5 h-10 border border-border text-foreground bg-transparent px-2 rounded-md hover:bg-transparent shadow-sm">
      <UserAvatarCard photo={mention.photo} altText={mention.name} sizeClass="h-7 w-7" />@{mention.name}
    </Badge>
  );
}
...
function ContextBadge({ icon, label }) {
  return (
    <Badge className="text-sm font-bold px-2.5 h-10 flex items-center gap-1.5 border rounded-md hover:bg-transparent border-foreground text-foreground bg-transparent">
...
function CommentItem({ comment, onEditComment, isContact = false }) {
  ...
  return (
    <div id={`comment-${comment.id}`} className="flex flex-col pb-4 mb-4 border-b  last:border-0 last:pb-0 last:mb-0">
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-2 min-w-0 flex-1">
          <div className="flex items-center gap-3 min-w-0">
            <UserAvatarCard photo={comment.author?.photo} altText={comment.author?.name} sizeClass="h-10 w-10" />
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 shrink-0">
          {/* Badge Creare */}
          <Badge
            className={` font-bold ${isContact ? "text-[10px] h-8 gap-1 px-2" : " px-2.5 gap-1.5 h-10 text-sm"}   flex items-center  border border-border bg-transparent hover:bg-transparent shadow-sm`}
          >
          ...
          {comment.editor?.name && (
            <div className="relative group flex items-center">
              <Badge
                className={` font-bold ${isContact ? "text-[10px] h-8 gap-1 px-2" : " px-2.5 gap-1.5 h-10 text-sm"}   flex items-center  border border-border bg-transparent hover:bg-transparent shadow-sm cursor-help transition-colors group-hover:bg-muted/50`}
              >
                ...
                <FontAwesomeIcon icon={faCircleInfo} className={`text-primary ${isContact ? "text-sm" : "text-xl"}`} />
              </Badge>

              {/* Tooltip-ul propriu-zis care apare doar la hover */}
              <div className="absolute right-0 top-full mt-2 hidden group-hover:flex items-center gap-3 bg-card border border-border shadow-lg p-2.5 rounded-lg z-[100] min-w-[12rem] animate-in fade-in zoom-in-95 duration-200">
                <UserAvatarCard photo={comment.editor.photo} altText={comment.editor.name} sizeClass="h-9 w-9 rounded-md" />
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Editat de</span>
                  <span className="text-sm font-bold text-foreground leading-tight">{comment.editor.name}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Buton Editare */}
          <Button variant="ghost" size="icon" onClick={() => onEditComment(comment)} className={`${isContact ? "h-8 w-8" : "h-10 w-10"} text-muted-foreground hover:text-primary`}>
            <FontAwesomeIcon icon={faPenToSquare} className={`text-${isContact ? "base" : "lg"}`} />
          </Button>
        </div>
      </div>

      {/* Mesaj + Mentiuni - Aliniate cu Avatarul */}
      <div className="pl-[3.25rem] mt-1 flex flex-col gap-2">
        <p className="text-base text-foreground/90 leading-relaxed whitespace-pre-wrap">{comment.mesaj}</p>
...
function ActivityItemCard({ item, onEditActivity, onAddComment, onEditComment, usersList, isContact = false, targetFocus = null, clearFocus }) {
  ...
  return (
    <div
      id={`activity-${item.id}`}
      className={cn("rounded-xl border border-border shadow-sm transition-all hover:shadow-md hover:bg-muted/10 p-4 mb-4 scroll-mt-6 duration-500", getSeveritySurface(item.severitate || "medium"))}
    >
      {" "}
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 shrink-0">
          {/* Badge Creare */}
          <Badge className={`text-sm font-bold ${isContact ? "text-xs" : ""} px-2.5 h-10 flex items-center gap-1.5 border border-border bg-transparent hover:bg-transparent shadow-sm`}>
          ...
          {item.editor?.name && (
            <div className="relative group flex items-center">
              <Badge
                className={`text-sm font-bold ${isContact ? "text-xs" : ""} px-2.5 h-10 flex items-center gap-1.5 border border-border bg-transparent hover:bg-transparent shadow-sm  transition-colors group-hover:bg-muted/50`}
              >
                ...
                <FontAwesomeIcon icon={faCircleInfo} className={`text-primary ${isContact ? "text-base" : "text-xl"}`} />
              </Badge>

              {/* Tooltip-ul propriu-zis care apare doar la hover */}
              <div className="absolute right-0 top-full mt-2 hidden group-hover:flex items-center gap-3 bg-card border border-border shadow-lg p-2.5 rounded-lg z-[100] min-w-[12rem] animate-in fade-in zoom-in-95 duration-200">
                <UserAvatarCard photo={item.editor.photo} altText={item.editor.name} sizeClass="h-9 w-9 rounded-md" />
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Editat de</span>
                  <span className="text-sm font-bold text-foreground leading-tight">{item.editor.name}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Buton Editare */}
          <Button variant="ghost" size="icon" onClick={() => onEditActivity(item)} className="h-10 w-10 text-muted-foreground hover:text-primary">
            <FontAwesomeIcon icon={faPenToSquare} className="text-xl" />
          </Button>
        </div>
      </div>
      {/* Body Content (Mesaj + Mentiuni legate) */}
      <div className="pl-[3.75rem] flex flex-col gap-1.5 mt-1">
        <p className="text-base text-foreground/90 leading-relaxed whitespace-pre-wrap">{item.mesaj || <span className="italic text-muted-foreground">Fără mesaj.</span>}</p>
      ...
      <div className="pl-[3.75rem]">
        <div className="mt-4 pt-3 border-t ">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <FontAwesomeIcon icon={openComments ? faChevronUp : faChevronDown} className="text-base" />
              {openComments ? "Ascunde comentarii" : `Vezi comentarii (${item.comments_count || comments.length})`}
            </button>

            <Button variant="ghost" size="sm" onClick={() => onAddComment(item.id, item.severitate)} className="h-8 px-3 text-base font-semibold text-primary  hover:bg-card">
          ...
          {openComments && (
            <div className="pt-3">
              <div className="flex flex-col border-l-2 pl-4 ml-1">
                {isFetching && <SpinnerElement text={1} blurVisible={false} />}
                {!isFetching && comments.length === 0 && <p className="text-sm text-muted-foreground italic pb-2">Nu există comentarii încă.</p>}
  ...
  return (
    <>
      <Card className="border-0 shadow-sm h-full flex flex-col overflow-hidden">
        <CardHeader className="py-3 px-5 bg-muted/10 border-b shrink-0 z-10 flex flex-row items-center justify-between">
          <CardTitle className="text-base font-bold uppercase text-muted-foreground flex items-center gap-2 m-0">
      ...
      <CommentDialog
        ...
        parentSeverity={activeCommentActSeverity} // PROP NOUĂ
        usersList={usersList}
      />
    </>
  );
