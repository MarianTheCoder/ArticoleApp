...
export default function CompaniesMainPage() {
  ...
  const [draft, setDraft] = useState({
    ...
    email: "",
    telefon: "",
    nivel_strategic: "Tinta",
    status_relatie: "Prospect",
    nivel_risc: "Mediu",
    nda_semnat: false,
    scor_conformitate: 0,
    note: "",
        ...
        <div className="w-full bg-card grid grid-cols-[auto_1fr] rounded-lg px-8 p-4">
          ...
          <div className="relative justify-end w-full gap-2 xxl:gap-4 flex items-center">
            <div className="max-w-md relative w-full">
              ...
              <Input placeholder="Cauta companie dupa nume..." value={searchName} onChange={(e) => setSearchName(e.target.value)} className="pl-10" />
            </div>
        ...
        {companiesList.length > 0 ? (
          ...
          <div className="flex w-full h-full justify-center items-center">
            <span className="text-2xl text-muted-foreground italic">
              ...
              {!isFetching && searchNameDebounced.trim() == "" ? "Nu există nicio companie adăugată..." : !isFetching && `Nicio companie găsită pentru căutarea: "${searchNameDebounced.trim()}"`}
            </span>
          </div>
        )}
      </div>
