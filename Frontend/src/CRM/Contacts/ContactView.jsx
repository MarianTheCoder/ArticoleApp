...
export default function ContactView({ open, setOpen }) {
    ...
    const getActionLabel = (type) => {
        switch (type) {
            case 'edit': return 'Editat';
            case 'delete': return 'Șters';
            case 'create': return 'Adăugat';
            default: return 'Info';
        }
    };

    // Returns just the border color class for the thread line
    const getActionBorderColor = (type) => {
        switch (type) {
            case 'edit': return "border-medium";
            case 'delete': return "border-high";
            case 'create': return "border-low";
    ...
    const getActionBadgeStyle = (type) => {
        switch (type) {
            case 'edit': return "text-medium border-medium bg-transparent";
            case 'delete': return "text-high border-high bg-transparent";
            case 'create': return "text-low border-low bg-transparent";
            default: return "text-primary border-primary bg-transparent";
        }
    };
    ...
    const renderHistoryContent = (details) => {
        ...
        if (details.field && details.hasOwnProperty('old') && details.hasOwnProperty('new')) {
            ...
            if (details.field.includes('photo') || details.field.includes('logo')) {
                return <div className="text-base text-foreground flex items-center gap-2"><FontAwesomeIcon icon={faImage} /> Fotografie actualizată</div>;
            }
        ...
        if (typeof details === 'object') {
            const visibleEntries = Object.entries(details).filter(([key]) =>
                !['id', 'updated_by_user_id', 'created_by_user_id', 'companie_id'].includes(key)
            );
                ...
                <div className="mt-1 flex flex-col gap-1">
                    {visibleEntries.map(([key, value], idx) => {
                        if (key.includes('photo') || key.includes('logo')) {
                            return <div key={idx} className="text-base text-foreground flex items-center gap-2"><FontAwesomeIcon icon={faImage} /> Fotografie actualizată</div>;
                        }
    ...
    return (
        <Sheet open={!!open} onOpenChange={(val) => !val && setOpen(null)}>
            <SheetContent side="right" className="w-full sm:w-[40rem] gap-0 p-0 border-l border-border bg-background flex flex-col">
                ...
                <SheetHeader className="px-5 py-4 border-b border-border bg-card flex flex-row items-center gap-3 space-y-0 shrink-0">
                    ...
                    {contact.activ ? (
                        ...
                        <Badge variant="destructive" className="text-base px-3 py-0.5">Inactiv</Badge>
                    )}
                </SheetHeader>
                ...
                <Tabs defaultValue="detalii" className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto bg-background p-5">
                                ...
                                <CardContent className="p-4 flex flex-col gap-4">
                                    <div className="flex flex-col gap-2">
                                        <div className="flex items-center gap-3 text-base">
                                            <div className="w-5 flex justify-center text-muted-foreground"><FontAwesomeIcon icon={faEnvelope} /></div>
                                            {contact.email ? <a href={`mailto:${contact.email}`} className="text-foreground hover:text-primary hover:underline truncate font-medium">{contact.email}</a> : <span className="text-muted-foreground">—</span>}
                                        </div>
                                        <div className="flex items-center gap-3 text-base">
                                            <div className="w-5 flex justify-center text-muted-foreground"><FontAwesomeIcon icon={faPhone} /></div>
                                            {contact.telefon ? <a href={`tel:${contact.telefon}`} className="text-foreground hover:text-primary hover:underline font-medium">{contact.telefon}</a> : <span className="text-muted-foreground">—</span>}
                                        </div>
                                        <div className="flex items-center gap-3 text-base">
                                            <div className="w-5 flex justify-center text-muted-foreground"><FontAwesomeIcon icon={faLinkedin} /></div>
                                            {contact.linkedin_url ? <a href={contact.linkedin_url} target="_blank" rel="noreferrer" className="text-foreground hover:text-primary hover:underline font-medium truncate">Vezi Profil</a> : <span className="text-muted-foreground">—</span>}
                                        </div>
                                    </div>
                            ...
                            <Card className="border-border shadow-sm">
                                ...
                                <CardContent className="p-4">
                                    {contact.note ? (
                                        ...
                                        <div className="text-center text-base text-muted-foreground italic py-1">
                                            Nu există notițe salvate.
                                        </div>
                                    )}
                                </CardContent>
                            </Card>
                        ...
                        <TabsContent value="activități" className="mt-0 h-full">
                            <div className="flex items-center justify-center h-40 text-muted-foreground italic border-2 border-dashed border-border rounded-lg bg-muted/20">
                                Activități (Coming Soon)
                            </div>
                        </TabsContent>
                                                    ...
                                                    return (
                                                        <div key={item.id} className="flex flex-col">
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <div className="flex gap-2 items-center justify-between w-full">
                                                                    <div className="flex flex-wrap items-center gap-3 text-base">
                                                                        ...
                                                                        <Badge
                                                                            variant="secondary"
                                                                            className="text-sm font-semibold shadow-sm flex items-center gap-2 py-1 border-foreground/10 px-2"
                                                                        >
                                                            ...
                                                            <div className={`flex-1 ml-6 border-l-2 border-b-2 rounded-bl-3xl ${borderColor} pl-6 pb-6`}>
                                                                ...
                                                                {/* Message Body */}
                                                                {item.message && (
                                                                    <div className="text-base mt-1 text-foreground leading-relaxed">
                                                                        {item.message}
                                                                    </div>
                                                                )}
                                                                ...
                                                                {item.content && (
                                                                    <div className="mt-3 pl-3 border-l-2">
                                                                        {renderHistoryContent(item.content)}
                                                                    </div>
                                                                )}
                                                            </div>
                                                        </div>
