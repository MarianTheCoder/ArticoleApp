...
export default function AskDialog({ open, setOpen, title, description, onSubmit, onCancel, useCode = false }) {
    ...
    <Dialog open={!!open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-xl p-8 gap-8 border shadow-2xl">
        <DialogHeader className="flex flex-col items-center text-center gap-2">
          ...
          <div className="mb-4">
            <div className="bg-blue-200 dark:bg-blue-900/40 p-5 rounded-2xl flex items-center justify-center ring-1 ring-blue-700 dark:ring-blue-400 shadow-sm">
              <FontAwesomeIcon icon={faQuestionCircle} className="text-blue-700 dark:text-blue-400 text-5xl" />
            </div>
          </div>

          <div className="grid gap-2 w-full">
            <DialogTitle className="text-2xl font-semibold text-center text-foreground">{title || "Ești sigur?"}</DialogTitle>

            <DialogDescription className="text-base text-muted-foreground text-center leading-relaxed">{description || "Această acțiune este ireversibilă."}</DialogDescription>
          </div>
        </DialogHeader>
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-3 sm:justify-center w-full">
          <DialogClose asChild>
            <Button type="button" variant="outline" size="lg" onClick={handleCancel} className="text-base min-w-[120px]">
              Anulează
            </Button>
          </DialogClose>
          <Button
