...
export default function PhoneInputCustom({
  ...
  const handleInput = (e) => {
    ...
    let userDigits = onlyDigits.startsWith(dialDigits) ? onlyDigits.slice(dialDigits.length) : onlyDigits;
    ...
    <div className={`flex h-10 rounded-md border border-input  ${disabled ? "opacity-60 pointer-events-none" : ""} ${className}`}>
      ...
      <DropdownMenu onOpenChange={setDropOpen}>
        <DropdownMenuTrigger asChild>
          <button type="button" className="flex items-center gap-1.5 px-3 border-r border-input  transition text-sm shrink-0 rounded-l-md outline-none">
        ...
        <DropdownMenuContent align="start" className="w-44 p-1">
          {COUNTRIES.map((c) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground ">{c.dialCode}</span>
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
