import { faChevronDown, faX, faUser, faTrowelBricks, faTruck, faCar, faFolder, faCheck, faRotate } from "@fortawesome/free-solid-svg-icons";
...
export default function SarciniReteteOfertaModal({
  ...
  const fetchOfertePartsForThisSantier = async () => {
    try {
      ...
      const sorted = (res.data.parts ?? []).slice().sort((a, b) => a.name.localeCompare(b.name, "ro", { sensitivity: "base" }));
  ...
  const toggleSelectedChildren = async (parentId) => {
    ...
    try {
      ...
      console.log("Error fetching selected children:", err);
    }
  };
    ...
    <div className="w-[95%] h-[88%] containerAdauga text-base flex flex-col bg-white rounded-xl">
      ...
      <div className="flex items-center justify-between p-3 border-b border-gray-400">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <select value={ofertaPartId} onChange={(e) => setOfertaPartId(e.target.value)} className="border border-gray-400 rounded-lg p-2">
        ...
        <button onClick={() => setReteteOfertaModal(false)} className="rounded-lg p-2 bg-red-500 text-white hover:bg-red-600">
      ...
      {/* table */}
      {!ofertaPartId ? (
        <div className="flex-1 flex items-center justify-center text-lg text-gray-500 italic">Selectează o lucrare pentru a vedea rețetele disponibile.</div>
      ) : retete.length === 0 ? (
        <div className="flex-1 flex items-center justify-center text-lg text-gray-500 italic">Nicio rețetă găsită pentru această lucrare.</div>
      ) : (
        <>
          {/* TABLE AREA */}
          {viewMode === "selected" ? (
            ...
            <div className="flex-1 p-4 overflow-hidden">
              {selectedRetete.length === 0 ? (
                <div className="w-full h-full flex items-center justify-center text-gray-500 italic">Nicio rețetă selectată.</div>
              ) : (
                            ...
                            <tr className="h-14">
                              ...
                              <th className="border border-gray-400 p-2 text-center w-[15%]">
                                <div className="flex justify-between items-center">
                                  Articol
                                  <span
                                    onClick={() => setSelectedLimba((prev) => (prev === "RO" ? "FR" : "RO"))}
                                    className="mr-4 h-9 w-9 font-semibold  flex items-center justify-center select-none cursor-pointer rounded-full text-green-600 border-green-600 hover:border-green-700 hover:text-green-700 border"
                                  >
                                    {selectedLimba}
                                  </span>
                                </div>
                              ...
                              return (
                                <React.Fragment key={`sel-${r.id}`}>
                                  ...
                                  <tr className="bg-gray-100 hover:bg-gray-200" style={{ height: ROW_H }}>
                                    {/* chevron */}
                                    <td onClick={() => toggleSelectedChildren(r.id)} className="border border-gray-400 text-center hover:cursor-pointer" title={isOpen ? "Închide" : "Deschide"}>
                                      <FontAwesomeIcon icon={faChevronDown} className={`transition-transform text-lg ${isOpen ? "rotate-180" : ""}`} />
                                    </td>
                                    ...
                                    <td className="border border-gray-400 p-2">
                                      <div className="truncate" title={r?.detalii_aditionale || ""}>
                                        {r?.detalii_aditionale || ""}
                                      </div>
                                    </td>
                                    <td className="border border-gray-400 p-2">
                                      <div className="truncate" title={r?.reper_plan || ""}>
                                        {r?.reper_plan || ""}
                                      </div>
                                    </td>
                                    <td className="border border-gray-400 p-2">
                                      <div className="truncate" title={r?.furnizor || ""}>
                                        {r?.furnizor || ""}
                                      </div>
                                    </td>
                                    <td className="border border-gray-400 p-2">
                                    ...
                                    <td className="border relative border-gray-400 p-2">
                                      <OverflowPopover text={selectedLimba == "RO" ? r?.descriere || "" : r?.descriere_fr || ""} />
                                    </td>
                                    ...
                                    <td className="border border-gray-400 p-2 text-center hidden lg:table-cell">
                                      <div className="flex items-center justify-center">{r?.photo ? <img src={photoAPI + "/" + r.photo} alt="" className="h-8 w-12 object-fit rounded" /> : ""}</div>
                                    </td>
                                    ...
                                    <td className="border border-gray-400 p-2 text-center">
                                      <button onClick={() => removeSelected(r.id)} className="px-2 py-1 rounded text-white bg-rose-500 hover:bg-rose-600" title="Elimină rețeta din selecție">
                                        <FontAwesomeIcon icon={faX} />
                                      </button>
                                    </td>
                                  </tr>
                                    ...
                                    kids.map((c) => {
                                      ...
                                      return (
                                        <tr key={`sel-child-${r.id}-${c.id || c.def_id}`} className="bg-white">
                                          ...
                                          <td className="text-center" style={{ border: "1px solid", borderColor: "#d1d5db", borderLeft: "none", borderTop: "none", borderBottom: "none" }} />
                                          <td className="border border-gray-400 text-lg p-2 text-center">
                                          ...
                                          <td className="border border-gray-400 p-2" />
                                          <td className="border border-gray-400 p-2">
                                            <div className="truncate" title={c?.furnizor || ""}>
                                              {c?.furnizor || ""}
                                            </div>
                                          </td>
                                          <td className="border border-gray-400 p-2">
                                            <div className="truncate" title={c?.cod || ""}>
                                              {c?.cod || ""}
                                            </div>
                                          </td>
                                          <td className="border border-gray-400 p-2" />
                                          <td className="border relative border-gray-400 p-2">
                                            <OverflowPopover text={selectedLimba == "RO" ? c?.articol || "" : c?.articol_fr || ""} />
                                          </td>
                                          <td className="border relative border-gray-400 p-2">
                                            <OverflowPopover text={selectedLimba == "RO" ? c?.descriere || "" : c?.descriere_fr || ""} />
                                          </td>
                ...
                <table className="w-full table-fixed text-sm">
                  <thead className="sticky top-0 bg-gray-300 text-black z-10">
                    <tr className="h-16">
                      ...
                      <th className="border border-gray-400 p-2 text-center w-[5%]">
                        <div className="truncate">{reper1 || "Reper 1"}</div>
                      </th>
                      <th className="border border-gray-400 p-2 text-center w-[5%]">
                        <div className="truncate">{reper2 || "Reper 2"}</div>
                      </th>
                      <th className="border border-gray-400 p-2 text-center w-[5%] hidden xl:table-cell">
                        <div className="truncate">Furnizor</div>
                      </th>
                      <th className="border border-gray-400 p-2 text-center w-[6.5rem]">
                        <div className="truncate">Cod</div>
                      </th>
                      <th className="border border-gray-400 p-2 text-center w-[10%] hidden lg:table-cell">
                        <div className="truncate">Articol Client</div>
                      </th>
                      <th className="border border-gray-400 p-2 text-left w-[12%]">
                        <div className="flex justify-between items-center">
                          Articol
                          <span
                            onClick={() => setSelectedLimbaMain((prev) => (prev === "RO" ? "FR" : "RO"))}
                            className="mr-4 h-9 w-9 font-semibold  flex items-center justify-center select-none cursor-pointer rounded-full text-green-600 border-green-600 hover:border-green-700 hover:text-green-700 border"
                          >
                            {selectedLimbaMain}
                          </span>
                      ...
                      <th className="border border-gray-400 p-2 text-center w-[12%] hidden xl:table-cell">
                        <div className="truncate">Descriere</div>
                      </th>
                  ...
                  <tbody>
                    {virtualItems.length > 0 && (
                      <tr>
                        <td colSpan={17} style={{ height: virtualItems[0].start }} />
                      </tr>
                    )}
                    ...
                    {virtualItems.map((vi) => {
                      ...
                      const rowTint = isChild ? "bg-white" : showFullRed ? "bg-red-100" : selected ? "bg-blue-100" : "bg-gray-100 hover:bg-gray-200";

                      return (
                        <tr key={`${r.parentId ? `c-${r.parentId}-` : ""}${r.id}-${vi.index}`} className={rowTint} style={{ height: ROW_H }}>
                          {/* expand/collapse */}
                          <td
                            onClick={() => {
                            ...
                            style={
                              isChild ? { border: "1px solid", borderColor: "#99a1af", borderLeft: "none", borderTop: "none", borderBottom: "none" } : { border: "1px solid", borderColor: "#99a1af" }
                            }
                          >
                            {isChild ? (
                              <span />
                              ...
                              <div className="w-full h-full rounded" title={chevronOpen ? "Închide" : "Deschide"}>
                                <FontAwesomeIcon icon={faChevronDown} className={`transition-transform text-lg ${chevronOpen ? "rotate-180" : ""}`} />
                              </div>
                            )}
                          </td>
                          ...
                          <td className="border border-gray-400 p-2 hidden lg:table-cell">
                            <div className="flex items-center justify-center h-full">{r?.photo ? <img src={photoAPI + "/" + r.photo} alt="" className="h-8 w-12 object-fit rounded" /> : ""}</div>
                          </td>
                                ...
                                {showFullRed ? (
                                  ...
                                  <>
                                    <div className="w-full h-2 rounded bg-slate-400 overflow-hidden flex">
                                      ...
                                      {addPart > 0 && <div className="h-full bg-blue-500" style={{ width: `${pctBlue}%` }} />}
                                    </div>
                            ...
                            {isChild ? (
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    if (!selected) setAllocInput(r.id, e.target.value);
                                  }}
                                  className={`w-20 rounded px-2 py-1 text-right text-xs ${selected ? "bg-gray-100 border border-gray-300 cursor-not-allowed" : "border border-gray-400"}`}
                                  readOnly={selected}
                                />
                                <button
          ...
          <div className=" w-full text-base items-center flex gap-4  p-4">
            ...
            <div className="flex justify-center items-center gap-2 p-4 bg-gray-200 rounded-full px-12">
              Rețete selectate: <span className=" font-semibold">{selectedRetete.length}</span>
            </div>
            <div className="flex justify-center items-center gap-2 p-4 bg-gray-200 rounded-full px-12">
              Timp estimat: <span className=" font-semibold">{estimatedTime ? estimatedTime : "N/A"}</span>
            </div>
            <div className="flex justify-center items-center gap-2 p-4 bg-gray-200 rounded-full px-12">
              Cost estimat: <span className=" font-semibold">{estimatedCost ? estimatedCost : "N/A"}</span>
            </div>
            <button
              onClick={() => setReteteOfertaModal(false)}
              disabled={selectedRetete.length === 0}
              className={`text-white flex-1 ${selectedRetete.length > 0 ? "bg-green-500 hover:bg-green-600" : "bg-gray-400 cursor-not-allowed"} p-4 font-semibold rounded-full`}
            >
              Confirmă Selecția
            </button>
          </div>
