...
export default function SarciniReteteModal({ setReteteModal, selectedRetete = [], onSelectionChange = () => {} }) {
  ...
  const [selectedIds, setSelectedIds] = useState(() => new Set(selectedRetete.map((r) => r.id)));
  ...
  const canFetch = clasa.trim().length >= 2 || cod.trim().length >= 2 || articol.trim().length >= 2;
  ...
  const fetchLight = async () => {
    ...
    try {
      ...
      if (Object.keys(seededAlloc).length) setAllocationInputs((prev) => ({ ...seededAlloc, ...prev }));
      if (Object.keys(seededReper).length) setReperInputs((prev) => ({ ...seededReper, ...prev }));
    } catch (e) {
      console.log("fetchLight error:", e);
      setRows([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const toggleChildren = async (parentId) => {
    ...
    try {
      ...
      console.log("toggleChildren (DB) error:", err);
    }
  };
    ...
    <div className="w-[95%] h-[88%] containerAdauga text-base flex flex-col bg-white rounded-xl">
      ...
      <div className="flex items-center justify-between p-3 border-b border-gray-400">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <input className="border border-gray-300 rounded px-3 py-1 w-56" value={cod} onChange={(e) => setCod(e.target.value)} />
          <label className="text-sm">Clasa:</label>
          <input className="border border-gray-300 rounded px-3 py-1 w-80" value={clasa} onChange={(e) => setClasa(e.target.value)} />

          <label className="text-sm">Articol:</label>
          <input className="border border-gray-300 rounded px-3 py-1 w-[36rem]" placeholder="Caută articol" value={articol} onChange={(e) => setArticol(e.target.value)} />
          <button
        ...
        <button onClick={() => setReteteModal(false)} className="rounded-lg p-2 bg-red-500 text-white hover:bg-red-600">
      ...
      {selectedReteteModal ? (
        <div className="flex-1 p-4 overflow-hidden">
          <SelectedReteteModal selectedRetete={selectedRetete} onRemove={handleRemoveSelected} />
        </div>
      ) : !canFetch ? (
        <div className="flex-1 flex items-center justify-center text-lg text-gray-500 italic">Introdu minim 2 caractere într-unul din câmpurile de căutare.</div>
      ) : isLoading ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="h-16 w-16 rounded-full border-4 border-blue-500 border-t-transparent animate-spin" />
        </div>
      ) : rows.length === 0 ? (
        <div className="flex-1 flex items-center justify-center text-lg text-gray-500 italic">Nicio rețetă găsită.</div>
      ) : (
        <div className="flex-1 p-4 overflow-hidden">
            ...
            <table className="w-full table-fixed text-sm">
              ...
              <tbody>
                ...
                {virtualItems.map((vi) => {
                  ...
                  const hasInside = !isChild && (r.has_manopera > 0 || r.has_materiale > 0 || r.has_utilaje > 0 || r.has_transport > 0);

                  const costUnit = Number(r?.pret_total ?? r?.cost ?? 0);

                  const savedAlloc = allocationInputs[r.id] ?? "";
                  const draftAlloc = activeRowId === r.id && editDraft ? (editDraft.alloc ?? "") : null;
                  const inputRaw = draftAlloc ?? savedAlloc;

                  const chevronOpen = openDropdowns.has(r.id);
                  const selected = !isChild && selectedIds.has(r.id);

                  const rowTint = isChild ? "bg-white" : selected ? "bg-blue-100" : "bg-gray-100 hover:bg-gray-200";

                  const rep = reperInputs[r.id] || { reper1: "", reper2: "" };
                  const draftRep1 = activeRowId === r.id && editDraft ? editDraft.reper1 : rep.reper1;
                  const draftRep2 = activeRowId === r.id && editDraft ? editDraft.reper2 : rep.reper2;
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {isChild ? <span /> : <FontAwesomeIcon icon={faChevronDown} className={`transition-transform text-lg ${chevronOpen ? "rotate-180" : ""}`} />}
                      </td>
                      ...
                      {/* Reper 1 (cell) */}
                      <td className="border p-2 border-gray-400 h-full relative" onClick={(e) => e.stopPropagation()}>
                      ...
                      {/* Reper 2 (cell) */}
                      <td className="border p-2 border-gray-400 text-center relative" onClick={(e) => e.stopPropagation()}>
                      ...
                      <td className="border border-gray-400 p-2 text-center">{r?.clasa || ""}</td>
                      ...
                      <td className="border p-2 border-gray-400 font-semibold text-right">{fmtQty(r?.cantitate || 1)}</td>

                      <td className="border p-2 border-gray-400 font-semibold text-right">{fmtMoney(costUnit)}</td>

                      <td className="border p-2 border-gray-400 font-semibold text-right">{fmtMoney(costUnit * (r?.cantitate || 1))}</td>

                      {/* Alocat — real-time (draft or saved) */}
                      <td className="border p-2 text-blue-500 border-gray-400 font-semibold text-right">{(inputRaw * costUnit).toFixed(2) || ""}</td>

                      {/* Alocare */}
                      <td className="border p-2 border-gray-400" onClick={(e) => e.stopPropagation()}>
                        {isChild ? (
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (activeRowId === r.id && editDraft) {
                                  setEditDraft((prev) => (prev ? { ...prev, alloc: v } : prev));
                                } else {
                                  // if user types before row active, still mirror
                                  setAllocInput(r.id, v);
                                }
                              }}
                              className={`w-24 rounded font-semibold text-blue-500 px-2 py-1 text-right text-xs ${
                                selected ? "bg-gray-100 border border-gray-300 cursor-not-allowed" : "border border-gray-400"
                              }`}
                              readOnly={selected}
                              placeholder="0.000"
                            />
