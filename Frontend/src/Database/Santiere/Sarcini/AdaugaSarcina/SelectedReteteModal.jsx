...
import { faUser, faTrowelBricks, faTruck, faCar, faFolder, faChevronDown, faX } from "@fortawesome/free-solid-svg-icons";
...
export default function SelectedReteteModal({ selectedRetete = [], onRemove = () => {} }) {
  ...
  const toggleChildren = async (parentId) => {
    ...
    try {
      ...
      console.log("toggleChildren (Selected) error:", err);
    }
  };
  ...
  if (!selectedRetete?.length) {
    return <div className="w-full h-full flex items-center justify-center text-gray-500 italic">Nicio rețetă selectată.</div>;
  }
        ...
        <table className="w-full table-fixed text-sm">
          <thead className="sticky top-0 bg-gray-300 text-black z-10">
            <tr className="h-16">
              ...
              <th className="border border-gray-400 p-2 text-center w-[3rem]" />
              {/* X at far right */}
            </tr>
          </thead>
            ...
            {rowsToRender.map((r) => {
              ...
              return (
                <tr key={`sel-${isChild ? `c-${r.parentId}-` : ""}${r.id}`} className={`${isChild ? "bg-white" : "bg-gray-100"}`}>
                  ...
                  <td className="border p-2 border-gray-400 font-semibold text-right">{fmtQty(cantitate || 1)}</td>
                  ...
                  <td className="border p-2 border-gray-400 text-blue-500 font-semibold text-right">{fmtMoney(costUnit * allocQty)}</td>
                  <td className="border p-2 border-gray-400 text-blue-500 font-semibold text-right">{fmtQty(allocQty || 1)}</td>

                  {/* X at far right for top-level ONLY */}
                  <td className="border border-gray-400 p-2 text-center">
                    {!isChild ? (
                      <button className="px-2 py-1 rounded bg-rose-500 hover:bg-rose-600 text-white" title="Elimină din selecție" onClick={() => onRemove(r.id)}>
