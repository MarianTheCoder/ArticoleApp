...
import { faBars, faBrush, faDownload, faFolderOpen, faPlus, faCheck, faXmark, faHashtag, faLocationDot, faRotate, faMap, faRuler, faRoad, faCirclePlus } from "@fortawesome/free-solid-svg-icons";
...
// Hooks
import { usePins, useZones, useZonePatterns, useCreatePin, useUpdatePin, useDeletePin, useMarkPinSeen, usePreviewPlanReplacement, useCommitPlanReplacement } from "@/hooks/Database/useRezerve";
...
export default function PlanViewKonva({ plan, onPlanReplaced, onSelectManagementZone, onSelectDrawingZone }) {
  ...
  const { measureMode, measureDraft, measureHover, toggleMeasureMode, handleMeasureClick, handleMeasureMove } = useMeasureTool(plan, stageRef, viewerRef, OpenSeadragon, ui);

  const { handleWheel, handleMouseDown, handleMouseMove, handleMouseUp, handleMouseLeave, wasPanning } = useOSDInteraction(stageRef, viewerRef, OpenSeadragon, measureMode);
  ...
  const loadingGlobal = loadingPins || createPin.isPending || updatePin.isPending || deletePinMutation.isPending || previewReplacement.isPending || commitReplacement.isPending;

  // ==================== EFFECTS ====================

  // Adaugă acest efect alături de celelalte efecte (lângă reset on plan change)
  useEffect(() => {
  ...
  // ==================== HANDLERS ====================
  // Pin Operations
  const openPinFromCanvas = useCallback(
    async (p) => {
  ...
  const handleSelectPinFromMenu = useCallback(
    (p) => {
  ...
  const jumpToPin = useCallback(
    (p) => {
      ...
      const imgPoint = new OpenSeadragon.Point(p.x_pct * (plan?.width_px || 1), p.y_pct * (plan?.height_px || 1));
  ...
  const handleSaveEditedPin = async (patch) => {
    ...
    fd.append("assigned_user_id", patch.assigned_user_id == null || patch.assigned_user_id === "" ? "" : String(patch.assigned_user_id));
  ...
  const markAllPinsSeen = async () => {
    ...
    try {
      ...
      await Promise.all(unseenPins.map((p) => markPinSeen.mutateAsync({ planId: plan.id, pinId: p.id, userId: user.id, santierId: idSantier, isUnseen: true })));
  ...
  useEffect(() => {
    ...
    if (measureMode && measureDraft.length) {
      ...
      const textLines = [currSegM != null ? `Line: ${formatMeters(currSegM)}` : null, "----------------------", `Total: ${formatMeters(lenM)}`].filter(Boolean);
  ...
  return (
    <div className="h-full w-full relative">
      {/* Hidden file input */}
      <input type="file" ref={fileInputRef} accept="application/pdf" className="hidden" onChange={handlePickFile} />
      ...
      <div className="absolute top-4 flex flex-col items-start gap-3 left-4 z-10">
        <Card className="shadow-lg rounded-sm h-12 border-foreground bg-card">
          <CardContent className=" flex p-0 px-4 h-full items-center gap-2 text-sm font-medium text-foreground">
            ...
            <span className="flex items-center gap-1.5">
              Pini:{" "}
              <Badge variant="default" className="p-1 px-2 hover:bg-primary">
                {pins.length}
              </Badge>
            </span>
            <span className="text-muted-foreground">•</span>
        ...
        <div className="flex gap-2 items-center">
          <Button variant="outline" size="icon" onClick={() => refetchPins()} className="shadow-lg relative group rounded-sm bg-card h-12 w-12 border-foreground hover:bg-accent text-foreground">
            <FontAwesomeIcon className="hover:rotate-180 transition-all duration-300" icon={faRotate} />
          ...
          <Button variant="outline" size="icon" onClick={() => markAllPinsSeen()} className="shadow-lg relative group rounded-sm bg-card h-12 w-12 border-foreground hover:bg-accent text-foreground">
            <FontAwesomeIcon className="hover:rotate-180 transition-all duration-300" icon={faEye} />
          ...
          <Button variant="outline" onClick={() => onSelectDrawingZone(true)} className="shadow-lg rounded-sm bg-card h-12 text-foreground border-foreground hover:bg-primary transition-colors gap-2">
        ...
        {!replaceMode ? (
          ...
          <>
            <div className="gap-2 flex items-center text-base text-foreground bg-card border border-foreground px-3 py-2 h-12 rounded shadow-lg">
              ...
              <Input type="range" min={0.1} max={1} step={0.05} value={overlayInfo.opacity} onChange={(e) => setOverlayInfo((o) => ({ ...o, opacity: Number(e.target.value) }))} />
            </div>
          ...
          <>
            ...
            >
                  ...
                  zones.map((z) => {
                    ...
                    return (
                      <Group key={z.id} listening={false}>
                        <Line points={z.points} closed fill={fillColor} stroke="black" strokeWidth={z.strokeWidth ?? 3} shadowForStrokeEnabled={false} perfectDrawEnabled={false} />
                      </Group>
                    );
                  })}
              </Layer>
              ...
              >
                ...
                {!replaceMode && ghost && ghost.x != null && ghost.y != null && <Pin x={ghost.x} y={ghost.y} label={ghost.code} color="#8B5CF6" opacity={0.6} invScale={1 / (scaleRef.current || 1)} />}
              </Layer>
            ...
            {/* HUD Stage (Measure) */}
            <Stage ref={hudRef} width={size.w} height={size.h} listening={false} className="absolute inset-0 pointer-events-none">
        ...
        <Dialog open={ui.zonePanelOpen} onOpenChange={(open) => setUi((prev) => ({ ...prev, zonePanelOpen: open }))}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>Lista modelelor salvate pentru acest șantier.</DialogDescription>
            </DialogHeader>

            <div className="mt-4 max-h-[400px] overflow-y-auto space-y-3 pr-1">
              {allZones.length === 0 && <div className="text-center py-8 text-muted-foreground">Nu există încă pattern-uri salvate pentru acest șantier.</div>}

              {allZones.map((patt) => {
                const isActive = patt.id === currentPatternId;
                return (
                  <Card key={patt.id} className={`transition-all ${isActive ? "border-primary bg-primary/5 shadow-md" : "border-border hover:bg-accent/50"}`}>
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="text-base font-semibold text-foreground">{patt.name || `Pattern #${patt.id}`}</h4>
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className="text-sm">
                            {patt.zones_count ?? 0} zone
                          </Badge>
                          {isActive && <Badge className="text-sm bg-primary text-primary-foreground">Curent</Badge>}
                        </div>
                      </div>
                      {patt.description && <p className="text-sm text-muted-foreground leading-relaxed">{patt.description}</p>}
                    </CardContent>
                  </Card>
                );
              })}
            ...
            <div className="mt-6 flex justify-between items-center gap-3">
              <Button onClick={() => onSelectManagementZone(true)} className="gap-2">
                <FontAwesomeIcon icon={faPlus} />
                Creează o zonă nouă
              </Button>
              <Button variant="outline" onClick={() => setUi((prev) => ({ ...prev, zonePanelOpen: false }))}>
                Închide
              </Button>
            </div>
          </DialogContent>
