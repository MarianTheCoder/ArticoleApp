...
import { faEdit, faEllipsis, faFilePdf, faTrash, faPlus, faChevronDown, faChevronRight, faCube, faFileLines } from "@fortawesome/free-solid-svg-icons";
...
// React Query hooks
import { useLucrari, useAddLucrare, useAddLucrare3D, useEditLucrare, useDeleteLucrare, useUploadPlan, useEditPlan, useDeletePlan } from "@/hooks/Database/useRezerve";
...
export default function SidebarRezerve({ onSelectPlan, onSelectLucrare3D, selectedPlanSideBar }) {
  ...
  const selectedLucrare = useMemo(() => lucrari.find((l) => l.id === selectedLucrareId) || null, [lucrari, selectedLucrareId]);
  ...
  return (
    <div className="h-full w-full flex border rounded-lg flex-col bg-card">
              ...
              <DialogContent>
                <form onSubmit={handleAddLucrare}>
                  <DialogHeader>
                    ...
                    <DialogDescription>Introduceți numele lucrării și o descriere opțională.</DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      <Label htmlFor="name">
                        Nume lucrare <span className="text-high">*</span>
                      </Label>
                      <Input id="name" value={newLucrare.name} onChange={(e) => setNewLucrare((prev) => ({ ...prev, name: e.target.value }))} placeholder="Ex: Plan parter" />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="desc">Descriere</Label>
                      <Textarea id="desc" value={newLucrare.description} onChange={(e) => setNewLucrare((prev) => ({ ...prev, description: e.target.value }))} rows={3} />
                    </div>
                  </div>
                  <DialogFooter>
                    <DialogClose asChild>
              ...
              <DialogContent>
                <form onSubmit={handleAddLucrare3D}>
                  <DialogHeader>
                    ...
                    <DialogDescription>Încărcați un model 3D (GLB).</DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      <Label htmlFor="name3d">
                        Nume lucrare <span className="text-high">*</span>
                      </Label>
                      <Input id="name3d" value={newLucrare3D.name} onChange={(e) => setNewLucrare3D({ ...newLucrare3D, name: e.target.value })} placeholder="Ex: Model clădire" />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="desc3d">Descriere</Label>
                      <Textarea id="desc3d" value={newLucrare3D.description} onChange={(e) => setNewLucrare3D({ ...newLucrare3D, description: e.target.value })} rows={2} />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="file3d">
                        Fișier model <span className="text-high">*</span>
                      </Label>
                      <Input id="file3d" type="file" accept=".glb" className="cursor-pointer" onChange={(e) => setNewLucrare3D({ ...newLucrare3D, file: e.target.files?.[0] || null })} />
              ...
              <DialogContent>
                <form onSubmit={handleUploadPlan}>
                  <DialogHeader>
                    ...
                    <DialogDescription>Selectați un fișier PDF și completați detaliile.</DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      <Label htmlFor="title">
                        Titlu plan <span className="text-high">*</span>
                      </Label>
                      <Input id="title" value={uploadPlan.title} onChange={(e) => setUploadPlan({ ...uploadPlan, title: e.target.value })} placeholder="Ex: Plan parter" />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="grid gap-2">
                        <Label htmlFor="scale">Scară</Label>
                        <Input id="scale" value={uploadPlan.scale} onChange={(e) => setUploadPlan({ ...uploadPlan, scale: e.target.value })} placeholder="1:50" />
                      </div>
                      <div className="grid gap-2">
                        <Label htmlFor="dpi">DPI</Label>
                        <Input id="dpi" type="number" value={uploadPlan.dpi} onChange={(e) => setUploadPlan({ ...uploadPlan, dpi: Number(e.target.value) })} placeholder="300" />
                      </div>
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="file">
                        Fișier PDF <span className="text-high">*</span>
                      </Label>
                      <Input id="file" type="file" accept="application/pdf" className="cursor-pointer" onChange={(e) => setUploadPlan({ ...uploadPlan, file: e.target.files?.[0] || null })} />
                    </div>
                  </div>
                  <DialogFooter>
                    <DialogClose asChild>
          ...
          <form onSubmit={handleEditLucrare}>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input id="edit-name" value={editingLucrare?.name || ""} onChange={(e) => setEditingLucrare((prev) => (prev ? { ...prev, name: e.target.value } : null))} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-desc">Descriere</Label>
                <Textarea id="edit-desc" value={editingLucrare?.description || ""} onChange={(e) => setEditingLucrare((prev) => (prev ? { ...prev, description: e.target.value } : null))} rows={3} />
              </div>
            </div>
            <DialogFooter>
              <DialogClose asChild>
      ...
      <Dialog open={openEditPlan} onOpenChange={setOpenEditPlan}>
        <DialogContent>
          <form onSubmit={handleEditPlan}>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label htmlFor="edit-plan-title">
                  Titlu plan <span className="text-high">*</span>
                </Label>
                <Input id="edit-plan-title" value={editingPlan?.title || ""} onChange={(e) => setEditingPlan((prev) => (prev ? { ...prev, title: e.target.value } : null))} />
              </div>
            </div>
            <DialogFooter>
              <DialogClose asChild>
      ...
      />
      {fetchingLucrari && !loading && <SpinnerElement text={2} />}
    </div>
  );
}
