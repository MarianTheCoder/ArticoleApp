...
export default function MeasureInfo({ info, cursorPos }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="text-base uppercase tracking-widest text-slate-400 mb-2">Măsurători</div>

      <div className="flex flex-col gap-2 whitespace-nowrap">
        {/* Current segment length */}
        <div className="flex items-center gap-2">
          <img src={distance} alt="Segment length" className="w-6 h-6 opacity-80" />
          <div className="flex  items-center">
            <span className="text-base w-24 text-slate-300">Segment</span>
            <span className="text-base  text-emerald-300">{segLabel}</span>
          </div>
        </div>

        {/* Total length */}
        <div className="flex  items-center  gap-2">
          <img src={distanceTotal} alt="Total length" className="w-6 h-6 opacity-80" />
          <div className="flex items-center">
            <span className="text-base w-24 text-slate-300">Total</span>
            <span className="text-base  text-emerald-300">{totalLabel}</span>
          </div>
        </div>
        {/* Angle */}
        <div className="flex items-center  gap-2">
          <img src={angle} alt="Angle" className="w-6 h-6 opacity-80" />
          <div className="flex items-center">
            <span className="text-base w-24 text-slate-300">Unghi</span>
            <span className="text-base  text-emerald-300">{angleDeg.toFixed(1)}°</span>
          </div>
        </div>
      </div>
    </div>
