...
export default function GlbViewer({ plan }) {
  ...
  if (!plan) {
    return <div className="w-full h-full flex items-center justify-center text-gray-500">Niciun model 3D selectat.</div>;
  }
  if (!plan.asset_path) {
    return (
  ...
  return (
    <div className="relative w-full h-full">
      ...
      >
        ...
        <input type="color" value={bg} onChange={(e) => setBg(e.target.value)} style={{ width: "100%", height: 32, padding: 0, border: "1px solid #ddd", borderRadius: 6 }} />

        <label style={{ display: "block", margin: "10px 0 4px" }}>
          Ambient light: <b>{ambient.toFixed(2)}</b>
        </label>
        <input type="range" min={0} max={2} step={0.05} value={ambient} onChange={(e) => setAmbient(parseFloat(e.target.value))} style={{ width: "100%" }} />

        <label style={{ display: "block", margin: "10px 0 4px" }}>
          Directional light: <b>{dir.toFixed(2)}</b>
        </label>
        <input type="range" min={0} max={3} step={0.05} value={dir} onChange={(e) => setDir(parseFloat(e.target.value))} style={{ width: "100%" }} />
        ...
        <input type="range" min={10} max={120} step={1} value={speed} onChange={(e) => setSpeed(parseInt(e.target.value))} style={{ width: "100%" }} />

        <label style={{ display: "block", margin: "8px 0 4px" }}>
          Viteză maximă: <b>{maxSpeed}</b>
        </label>
        <input type="range" min={6} max={60} step={1} value={maxSpeed} onChange={(e) => setMaxSpeed(parseInt(e.target.value))} style={{ width: "100%" }} />
      </div>
    </div>
  );
}
