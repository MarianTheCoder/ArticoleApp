...
const MAX_COLUMN_VALUE_LENGTH = 15;
...
export default function OferteAddDialog({ open, setOpen, selectedOferta, selectedLucrare, onConfirm }) {
  ...
  }, [selectedOferta, selectedLucrare, selectedReteta, quantity, dynamicColumns, columnValues, onConfirm, handleOpenChange]);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-[90vw] h-[95vh] rounded-md p-0 gap-0 flex flex-col ">
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          {open && <ReteteMainPage isSelectionMode isEmbedded selectedRetetaId={selectedReteta?.id || null} onSelectReteta={setSelectedReteta} lockedLang={limbaUser} />}
        </div>

        <div className="bg-muted rounded-b-md min-h-[6.5rem] px-4 py-3 border-t border-border shrink-0 grid grid-cols-[0.75fr_auto_0.75fr] items-center gap-5">
          <div className=" ">
            {selectedReteta ? (
              <div className="min-w-0">
                ...
                <div className="mt-1">
                  <OverflowTooltip text={selectedReteta.descriere || "—"} maxLines={2} align="left" className="text-sm text-muted-foreground" />
                </div>
              </div>
            ) : (
          ...
          {dynamicColumns.length > 0 ? (
            <div className="flex-1 min-w-0 flex items-end justify-center gap-2">
              {dynamicColumns.map((col) => (
                <div key={col.id} className="flex flex-col gap-1 min-w-0 w-44">
                  <Label className="text-xs font-bold text-foreground truncate">{col.name}</Label>

                  <Input value={columnValues[col.id] || ""} onChange={(e) => setColumnValue(col.id, e.target.value)} maxLength={MAX_COLUMN_VALUE_LENGTH} placeholder="Max 15" className="h-9 text-sm" />
                </div>
              ))}
            </div>
          ) : (
            <div className="flex-1 min-w-0 flex items-end justify-center gap-2">
              <span className="italic text-sm text-muted-foreground">Această lucrare nu are coloane dinamice configurate.</span>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 justify-end shrink-0">
            ...
            {selectedReteta && (
              <Button variant="destructive" onClick={() => setSelectedReteta(null)} className="gap-2 h-11 px-8 font-bold shadow-md transition-all active:scale-95">
