...
import { useAtribuiri, addPontaj, usePontaje } from "@/hooks/Database/usePontaje";
...
const TIMEZONE_MAP = {
  "Europe/Bucharest": { code: "RO", name: "România", flag: "🇷🇴" },
  "Europe/Chisinau": { code: "MD", name: "Moldova", flag: "🇲🇩" },
  "Europe/Paris": { code: "FR", name: "Franța", flag: "🇫🇷" },
  "Europe/London": { code: "UK", name: "UK", flag: "🇬🇧" },
  "Europe/Berlin": { code: "DE", name: "Germania", flag: "🇩🇪" },
  "Europe/Madrid": { code: "ES", name: "Spania", flag: "🇪🇸" },
  "Europe/Rome": { code: "IT", name: "Italia", flag: "🇮🇹" },
};
...
export default function Pontaje() {
  ...
  const locationInfo = TIMEZONE_MAP[tzString] || {
    code: "INTL",
    name: tzString.split("/").pop().replace("_", " "),
    flag: "🌍",
  };

  // Calculăm offset-ul UTC (ex: +02:00 sau -05:00)
  const getUtcOffsetString = () => {
    const offsetMinutes = new Date().getTimezoneOffset(); // Atenție: returnează cu minus pentru fusurile de la est de UTC
    const absMinutes = Math.abs(offsetMinutes);
    const hours = String(Math.floor(absMinutes / 60)).padStart(2, "0");
    const minutes = String(absMinutes % 60).padStart(2, "0");
    const sign = offsetMinutes <= 0 ? "+" : "-"; // Inversăm semnul pentru display standard
