        ...
        queryFn: async () => {
            ...
            return rawZones.map((z) => {
                ...
                const ptsPx = pts.map((v, i) =>
                    i % 2 === 0 ? v * planWidth : v * planHeight
                );
...
export const useMarkPinSeen = () => {
    ...
    return useMutation({
        ...
        onMutate: async ({ planId, pinId, userId, santierId, isUnseen }) => {
            ...
            queryClient.setQueryData(['Plan', 'pins', String(planId), String(userId)], (old) =>
                old?.map(pin => pin.id === pinId ? { ...pin, is_unseen: 0 } : pin)
            );
        ...
        onError: (err, variables, context) => {
            if (context?.previousPins) {
                queryClient.setQueryData(
                    ['Plan', 'pins', String(variables.planId), String(variables.userId)],
