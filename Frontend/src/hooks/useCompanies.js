...
export const useCompanyHistory = (companyId, filialaId = null, santierId = null, contactId = null) => {
  return useQuery({
    // Cheia include searchName pentru a declanșa refetch automat la tastare
    queryKey: ["companies", "history", companyId, filialaId, santierId, contactId],

    queryFn: async () => {
      const { data } = await api.get(`/CRM/Notifications/history/company/${companyId}`, { params: { filialaId, santierId, contactId } });
      return data;
    },
