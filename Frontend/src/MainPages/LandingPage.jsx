...
function BeamBackground() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Grid
      ctx.strokeStyle = "rgba(0,130,255,0.05)";
      ctx.lineWidth = 1;
      ...
      // Center bloom
      const bloom = ctx.createRadialGradient(canvas.width * 0.5, canvas.height * 0.45, 0, canvas.width * 0.5, canvas.height * 0.45, canvas.width * 0.55);
    ...
    <div className="bg-[#04060d] text-white overflow-x-hidden min-h-screen" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-[7%]">
        ...
        <div className="relative z-10 flex flex-col items-center">
          {/* Badge */}
          <div className="a1 flex items-center gap-3 mb-10" style={{ fontSize: "1rem", fontWeight: 700, letterSpacing: "0.3rem", color: "#00A3FF", textTransform: "uppercase" }}>
          ...
          {/* Main title */}
          <h1 className="a2 uppercase leading-none mb-8" style={{ fontWeight: 900, fontSize: "clamp(4rem, 10vw, 8rem)", letterSpacing: "-0.1rem" }}>
            <span className="block text-white/95">Energie.</span>
            <span className="block" style={{ WebkitTextStroke: "0.1rem rgba(0,163,255,0.6)", color: "transparent" }}>
              Precizie.
            </span>
          ...
          {/* Subtitle */}
          <p className="a3 text-white/50 max-w-xl leading-relaxed mb-14" style={{ fontFamily: "'Barlow', sans-serif", fontWeight: 300, fontSize: "1.2rem" }}>
            Instalații electrice, sanitare și climatizare — execuție impecabilă pentru proiecte rezidențiale și industriale.
          </p>

          {/* Tags */}
          <div className="a4 flex flex-wrap justify-center gap-3">
            {["Electricitate", "Instalații Sanitare", "CVC", "Industrial", "Rezidențial"].map((t) => (
              <span key={t} className="border border-white/15 text-white/50 px-4 py-2" style={{ fontSize: "0.8rem", fontWeight: 700, letterSpacing: "0.18rem", textTransform: "uppercase" }}>
                {t}
              </span>
            ))}
          </div>
