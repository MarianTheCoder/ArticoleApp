...
export default function PontajeSantierTable({
  ...
  const rows = useMemo(() => {
    ...
    for (const a of assignments) {
      if (!assignedBySite.has(a.santier_id))
        assignedBySite.set(a.santier_id, new Set());
    ...
    const ensureBucket = (row, date) => {
      if (!row.dateBuckets.has(date))
        row.dateBuckets.set(date, { hasActive: false, minutes: 0 });
        ...
        for (const s of ws.sessions || []) {
          ...
          if (!rowsByKey.has(key)) {
            rowsByKey.set(key, {
              ...
              assignedUsers: new Set(
                siteId ? assignedBySite.get(siteId) || [] : [],
              ),
              startedUsers: new Set(),
              activeUsers: new Set(),
              loggedUsers: new Set(),
          ...
          row.sessions.push({
            ...
            user: {
              ...
              name: u.name,
              role: u.role,
              photo_url: u.photo_url || userMeta?.photo_url,
            },
          });
          ...
          const isActive =
            !s.end_time && (s.status === "active" || s.status == null);
          if (isActive) {
  ...
  const allOnPageSelected =
    visible.length > 0 && visible.every((x) => selectedIds.has(x.key));
  ...
  const toggleAllOnPage = () =>
    setSelectedIds((prev) => {
      const next = new Set(prev);
      const everySelected =
        visible.length > 0 && visible.every((x) => next.has(x.key));
        ...
        <div className="relative flex h-20 exportMenu gap-4 text-black items-center">
          {selectMode && (
            <>
              ...
              >
                {allOnPageSelected ? "Deselectează toți" : "Selectează toți"} (
                {selectedIds.size})
              </button>
            </>
          )}
          ...
          <button
            ...
            onClick={() => {
              if (selectMode) {
                if (!hasSelection) {
                  alert("Selectează cel puțin o activitate pentru export.");
                  return;
                }
          ...
          {exportMenuOpen && !selectMode && (
            <div
              role="menu"
              className="absolute right-0 top-16 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-30"
            >
                ...
                >
                  <td className="px-4 py-2 ">
                    <span
                      ...
                      className="p-2 rounded-full px-12"
                    >
                      {r.siteName}
                    </span>
                  </td>
                  ...
                  {/* show '-' when multiple days */}
                  <td className="px-4 py-2 font-semibold">
                    {selectedIsoDates.length > 1 ? "-" : r.activeUsers.size}
                  </td>
                  <td className="px-4 py-2 font-bold">
                    {fmtHHMM(r.totalMinutes)}
                  </td>
                </tr>
              ))}
            </tbody>
