...
export default async function ExportPontajeSantiere({ selectedSantierIds, dates, includeUnassigned = true }) {
  try {
    ...
    santiere.forEach((s) => {
      // (a) santier total row
      const usersAllGood = [...(s.users || []).map((u) => ({ ...u, _isExtra: false })), ...(s.extra_users || []).map((u) => ({ ...u, _isExtra: true }))];
      ...
      const usersAll = [...(s.users || []).map((u) => ({ ...u, _isExtra: false })), ...(s.extra_users || []).map((u) => ({ ...u, _isExtra: true }))];
