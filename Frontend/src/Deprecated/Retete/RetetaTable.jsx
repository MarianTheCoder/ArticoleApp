...
export default function ManoperaTable({
  ...
  const fetchManopere = async (offset, limit) => {
    try {
      ...
      console.log("Eroare la obtinerea de informatii", error);
    }
  };
  ...
  const handleEditCantitateInterior = async (passedRow) => {
    ...
    if (selectedEditCantitateInterior === key) {
      ...
      try {
        // send update
        await api.put(`/Retete/editCantitateInterior/${passedRow.original.reteta_id}/${passedRow.original.id}/${passedRow.original.whatIs}`, { cantitate: newQty });
        ...
        console.log("Error updating cantitate:", error);
      }
    } else {
    ...
    () => [
      {
        ...
        cell: ({ row, getValue, cell }) => (
          ...
          >
            <FontAwesomeIcon className={`text-center ${open.some((item) => item.id === cell.row.original.id) ? "rotate-90" : ""} text-xl`} icon={faChevronRight} />
          </div>
        ),
      },
      {
      ...
      },
      {
        ...
        header: (
          <div className="flex items-center w-[95%] justify-between text-black ">
            <span>Cod</span>
            <FontAwesomeIcon
              onClick={() => setAscendentCOD((prev) => (prev == false ? true : false))}
              className="text-xl border border-black p-2  rounded-full  cursor-pointer"
              icon={!ascendentCOD ? faArrowUpAZ : faArrowDownAZ}
            />
          </div>
        ),
      ...
      { accessorKey: "clasa", header: "Clasă", size: 300 },
      {
        ...
        cell: ({ getValue, row }) => <OverflowPopover maxLines={2} text={localLimba === "RO" ? getValue() : row.original.articol_fr} />,
        size: 500,
      },
      {
        ...
        cell: ({ getValue, row }) => <OverflowPopover maxLines={2} text={localLimba === "RO" ? getValue() : row.original.descriere_reteta_fr} />,
        size: 500,
      },
      {
        accessorKey: "whatIs",
      ...
      },
      {
        ...
        cell: ({ getValue, row }) => {
          ...
          if (row.original.reteta_id) {
            ...
            return <div className="w-full flex font-medium">1.000</div>;
          }
        },
        size: 70,
      },
          ...
          <div className="absolute group w-full">
            ...
            <div className="w-full select-none flex items-center justify-center">
              ...
              selectedEditCantitateInterior == `${row.original.id}-${row.original.reteta_id}-${row.original.whatIs}` ? (
                <button onClick={() => handleEditCantitateInterior(row)} className="bg-green-500 hover:bg-green-600 text-white font-semibold text-base px-3 py-3 rounded-lg">
                  Confirm
                </button>
              ) : (
              ...
              <div className="absolute z-10 left-0 -translate-x-[40%] bg-white border shadow-lg rounded-md w-40 p-2 flex flex-col items-center gap-1 opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-all duration-200 text-lg font-semibold text-gray-700">
                {row.original.whatIs ? (
                  <>
                    ...
                    <div onClick={(e) => deleteItem(e, row)} className="cursor-pointer w-full flex justify-start items-center rounded-md px-2 py-2 hover:bg-red-100 hover:bg-opacity-90">
            ...
            <table className="w-full text-[0.91rem]  border-separate border-spacing-0 ">
              <thead className="top-0 w-full sticky  z-10 ">
                <tr className="text-black">
                  ...
                  <th className="border-b border-r bg-white border-black">
                    <input type="cod" name="cod" value={filters.cod} onChange={handleInputChange} className="p-2 w-full outline-none  py-3" placeholder="Filtru Cod" />
                  </th>
                  <th className="border-b border-r bg-white border-black">
                    <select id="clasa" name="clasa" value={filters.clasa} onChange={handleInputChange} className=" p-2 text-sm w-full cursor-pointer outline-none py-3">
                  ...
                  <th className="border-b border-r bg-white border-black">
                    <input type="text" name="articol" value={filters.articol} onChange={handleInputChange} className="p-2 w-full outline-none  py-3" placeholder="Filtru Articol" />
                  </th>
                  ...
                  {table.getRowModel().rows.map((row, index, rows) =>
                    row.original.whatIs == "addButton" ? (
                      <tr key={row.original.id}>
                        <td></td>
                        <td
                          onClick={() => setIsPopupOpen(row.original.retetaIdForFetch)}
                          className="bg-green-400 p-1 px-3 hover:bg-green-500 cursor-pointer border-b border-r border-black select-none text-black"
                          colSpan={12}
                        >
