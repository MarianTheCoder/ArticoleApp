...
export default function RetetaMateriale({ setIsPopupOpen, open, isPopupOpen, setOpen, delPreviewReteta, fetchPreviewReteta }) {
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "cod_definitie", header: "Cod", size: 50 },
      {
        ...
        header: "Denumire",
        cell: ({ getValue, row }) => (selectedMaterialeIds.includes(row.original.id) ? <div className="">{row.original.denumire_fr || "..."}</div> : getValue()),
      },
      {
        accessorKey: "descriere",
        header: "Descriere",
        cell: ({ getValue, row }) => (selectedMaterialeIds.includes(row.original.id) ? <div className="">{row.original.descriere_fr || "..."}</div> : getValue()),
      },
      ...
      <div className=" flex flex-col h-full w-full overflow-hidden">
        ...
        <div className="grid font-medium grid-cols-[auto_0.7fr_auto_1fr_2fr] gap-4 p-4 pt-2 text-black containerWhiter w-full">
          <div className="flex w-full flex-col items-center ">
            ...
            <select id="limba" name="limba" value={materialeFilters.limba} onChange={handleChangeFilterMateriale} className=" p-2 w-full cursor-pointer rounded-lg shadow-sm outline-none py-2">
          ...
          <div className="flex flex-col w-full items-center ">
            <label className=" text-black">Tip</label>
            <select
              id="tip_material"
              name="tip_material"
          ...
          <div className="flex flex-col w-full items-center ">
            <label className=" text-black">Clasa</label>
            <select id="clasa" name="clasa" value={materialeFilters.clasa} onChange={handleChangeFilterMateriale} className=" px-2 outline-none text-center py-2  w-full  rounded-lg shadow-sm">
          ...
          <div className="flex flex-col w-full items-center ">
            <label className=" text-black">Denumire</label>
            <input
              type="text"
              id="denumire"
              name="denumire"
        ...
        <div className=" w-full flex flex-col h-full justify-between gap-4 overflow-hidden p-4 ">
          <div className="w-full h-full flex flex-col scrollbar-webkit overflow-auto  justify-between">
            <table className="w-full  border-separate border-spacing-0 ">
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <tr key={headerGroup.id} className="bg-white text-black text-left  font-bold select-none">
                    {headerGroup.headers.map((header) => (
                      <th
                        key={header.id}
                        className={`relative border-b border-r border-black   bg-white p-1 ${header.column.id === "threeDots" ? "text-center" : ""} `}
                        style={{
              ...
              {materiale == null || materiale.length == 0 ? (
                <tbody>
                  <tr>
                    <th className=" text-black border-b border-r border-black p-2 bg-white" colSpan={9}>
                ...
                <tbody className=" relative z-0">
                  {table.getRowModel().rows.map((row, index, rows) => (
                    <tr
                      key={row.id}
                      onClick={() => setSelectedMateriale((prev) => (prev?.original.id == row.original.id ? null : row))}
                      className={` dropdown-container  text-black ${selectedMateriale?.original.id != row.original.id ? (row.index % 2 === 0 ? "bg-[rgb(255,255,255,0.75)] hover:bg-[rgb(255,255,255,0.65)]  " : "bg-[rgb(255,255,255,1)] hover:bg-[rgb(255,255,255,0.90)]") : "bg-blue-300 hover:bg-blue-400"} `}
                    >
          ...
          <div className="flex gap-4 w-full">
            ...
            <button onClick={() => handleAddItem()} className="bg-green-500 flex dropdown-container items-center justify-center gap-2 text-black flex-grow hover:bg-green-600  px-6 py-2 rounded-xl">
