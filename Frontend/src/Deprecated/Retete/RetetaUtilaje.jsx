...
export default function RetetaUtilaje({ setIsPopupOpen, open, isPopupOpen, setOpen, delPreviewReteta, fetchPreviewReteta }) {
  ...
  const fetchManopere = async () => {
    try {
      if (
        utilajeFilters.clasa_utilaj.trim().length >= 3 ||
        utilajeFilters.descriere.trim().length >= 3 ||
        utilajeFilters.utilaj.trim().length >= 3 ||
        utilajeFilters.cod_definitie.trim().length >= 3
      ) {
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "clasa_utilaj", header: "Clasa", size: 50 },
      {
        accessorKey: "utilaj",
        header: "Utilaj",
        cell: ({ getValue, row }) => (selectedUtilajeIds.includes(row.original.id) ? <div className="">{row.original.utilaj_fr || "..."}</div> : getValue()),
      },
      {
        accessorKey: "descriere",
        header: "Descriere",
        cell: ({ getValue, row }) => (selectedUtilajeIds.includes(row.original.id) ? <div className="">{row.original.descriere_fr || "..."}</div> : getValue()),
      },
      ...
      <div className=" flex flex-col h-full w-full overflow-hidden">
        ...
        <div className="grid font-medium grid-cols-[auto_1fr_auto_1fr_2fr] gap-4 p-4 pt-2 text-black containerWhiter w-full">
          <div className="flex w-full flex-col items-center ">
            ...
            <select id="limba" name="limba" value={utilajeFilters.limba} onChange={handleChangeFilterUtilaje} className=" p-2 w-full cursor-pointer rounded-lg shadow-sm outline-none py-2">
          ...
          <div className="flex w-full flex-col items-center ">
            <label className=" text-black">Utilaj</label>
            <input
              type="text"
              id="utilaj"
              name="utilaj"
          ...
          <div className="flex flex-col w-full items-center ">
            <label className=" text-black">Descriere</label>
            <input
              type="text"
              id="descriere"
              name="descriere"
          ...
          <div className="w-full h-full flex flex-col scrollbar-webkit overflow-auto  justify-between">
            <table className="w-full  border-separate border-spacing-0 ">
              ...
              {utilaje == null || utilaje.length == 0 ? (
                <tbody>
                  <tr>
                    <th className=" text-black border-b border-r border-black p-2 bg-white" colSpan={8}>
                ...
                <tbody className=" relative z-0">
                  {table.getRowModel().rows.map((row, index, rows) => (
                    <tr
                      key={row.id}
                      onClick={() => setSelectedUtilaje((prev) => (prev?.original.id == row.original.id ? null : row))}
                      className={` dropdown-container  text-black ${selectedUtilaje?.original.id != row.original.id ? (row.index % 2 === 0 ? "bg-[rgb(255,255,255,0.75)] hover:bg-[rgb(255,255,255,0.65)]  " : "bg-[rgb(255,255,255,1)] hover:bg-[rgb(255,255,255,0.90)]") : "bg-blue-300 hover:bg-blue-400"} `}
                    >
