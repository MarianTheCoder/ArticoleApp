...
export default function RetetaTransport({ setIsPopupOpen, open, isPopupOpen, setOpen, delPreviewReteta, fetchPreviewReteta }) {
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "clasa_transport", header: "Clasa", size: 100 },
      {
        ...
        header: "Transport",
        cell: ({ getValue, row }) => (selectedTransportIds.includes(row.original.id) ? <div className="">{row.original.transport_fr || "..."}</div> : getValue()),
        size: 200,
      },
      {
        accessorKey: "descriere",
        header: "Descriere",
        cell: ({ getValue, row }) => (selectedTransportIds.includes(row.original.id) ? <div className="">{row.original.descriere_fr || "..."}</div> : getValue()),
        size: 200,
      },
      ...
      <div className=" flex flex-col h-full w-full overflow-hidden">
        ...
        <div className="grid font-medium grid-cols-[auto_1fr_1fr_1fr] gap-4 p-4 pt-2 text-black containerWhiter w-full">
          <div className="flex w-full flex-col items-center ">
            ...
            <select id="limba" name="limba" value={transportFilters.limba} onChange={handleChangeFilterManopera} className=" p-2 w-full cursor-pointer rounded-lg shadow-sm outline-none py-2">
          ...
          <div className="flex flex-col w-full items-center ">
            <label className=" text-black">Transport</label>
            <input
              type="text"
              id="transport"
              name="transport"
              ...
              {transport == null || transport.length == 0 ? (
                ...
                <tbody className=" relative z-0">
                  {table.getRowModel().rows.map((row, index, rows) => (
                    <tr
                      key={row.id}
                      onClick={() => setSelectedTransport((prev) => (prev?.original.id == row.original.id ? null : row))}
                      className={` dropdown-container  text-black ${selectedTransport?.original.id != row.original.id ? (row.index % 2 === 0 ? "bg-[rgb(255,255,255,0.75)] hover:bg-[rgb(255,255,255,0.65)]  " : "bg-[rgb(255,255,255,1)] hover:bg-[rgb(255,255,255,0.90)]") : "bg-blue-300 hover:bg-blue-400"} `}
                    >
