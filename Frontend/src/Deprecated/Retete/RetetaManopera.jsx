...
export default function RetetaManopera({ setIsPopupOpen, open, isPopupOpen, setOpen, delPreviewReteta, fetchPreviewReteta }) {
  ...
  const fetchManopere = async () => {
    try {
      if (manoperaFilters.cod_definitie.trim().length >= 3 || manoperaFilters.ocupatie.trim().length >= 3) {
        const response = await api.get("/Manopera/FetchManopereLight", {
          params: {
            limba: manoperaFilters.limba,
            cod_definitie: manoperaFilters.cod_definitie, // Pass cod_definitie as a query parameter
    ...
    () => [
      {
        accessorKey: "limba",
        header: "Limba",
        cell: ({ getValue, row }) => <div className="flex font-semibold items-center justify-center">{getValue() || "RO&FR"}</div>,
        size: 20,
      },
      { accessorKey: "cod_definitie", header: "Cod", size: 50 },
      {
        ...
        header: "Ocupatie",
        cell: ({ getValue, row }) => (selectedManopereIds.includes(row.original.id) ? <div className="">{row.original.ocupatie_fr || "..."}</div> : getValue()),
      },
      {
        accessorKey: "descriere",
        header: "Descriere",
        cell: ({ getValue, row }) => (selectedManopereIds.includes(row.original.id) ? <div className="">{row.original.descriere_fr || "..."}</div> : getValue()),
      },
      ...
      { accessorKey: "cost_unitar", header: "Cost", size: 50 },
      {
        ...
        header: (
          <div className="flex w-full   justify-center">
            <div onClick={() => translateAll()} className="bg-blue-500 rounded-xl px-2 w-3/4 hover:bg-blue-600 hover:cursor-pointer flex gap-2 p-2 items-center justify-center">
      ...
      <div className=" flex flex-col h-full w-full overflow-hidden">
        ...
        <div className="grid font-medium grid-cols-[auto_1fr_1fr] gap-4 p-4 pt-2 text-black containerWhiter w-full">
          <div className="flex w-full flex-col items-center ">
            <label className=" text-black">Limba</label>
            <select id="limba" name="limba" value={manoperaFilters.limba} onChange={handleChangeFilterManopera} className=" p-2 w-full cursor-pointer rounded-lg shadow-sm outline-none py-2">
          ...
          <div className="flex w-full flex-col items-center ">
            ...
            <input
              ...
              id="cod_definitie"
              name="cod_definitie"
              value={manoperaFilters.cod_definitie}
          ...
          <div className="flex flex-col w-full items-center ">
            <label className=" text-black">Ocupatie</label>
            <input
              type="text"
              id="ocupatie"
              name="ocupatie"
        ...
        <div className=" w-full flex flex-col h-full justify-between gap-4 overflow-hidden p-4 ">
          <div className="w-full h-full flex flex-col scrollbar-webkit overflow-auto  justify-between">
            <table className="w-full  border-separate border-spacing-0 ">
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <tr key={headerGroup.id} className="bg-white text-black text-left  font-bold select-none">
                    {headerGroup.headers.map((header) => (
                      <th
                        key={header.id}
                        className={`relative border-b border-r border-black   bg-white p-2 ${header.column.id === "threeDots" ? "text-center" : ""} `}
                        style={{
              ...
              {manopere == null || manopere.length == 0 ? (
                <tbody>
                  <tr>
                    <th className=" text-black border-b border-r  border-black  p-2 bg-white" colSpan={7}>
                      Nici un Rezultat / Introdu minim 3 Caractere
                    </th>
                  </tr>
                </tbody>
              ) : (
                <tbody className=" relative z-0">
                  {table.getRowModel().rows.map((row, index, rows) => (
                    <tr
                      key={row.id}
                      onClick={() => setSelectedManopera((prev) => (prev?.original.id == row.original.id ? null : row))}
                      className={` dropdown-container  text-black ${selectedManopera?.original.id != row.original.id ? (row.index % 2 === 0 ? "bg-[rgb(255,255,255,0.75)] hover:bg-[rgb(255,255,255,0.65)]  " : "bg-[rgb(255,255,255,1)] hover:bg-[rgb(255,255,255,0.90)]") : "bg-blue-300 hover:bg-blue-400"} `}
                    >
          ...
          <div className="flex gap-4 w-full">
            ...
            <button onClick={() => handleAddItem()} className="bg-green-500 dropdown-container flex  items-center justify-center gap-2 text-black flex-grow hover:bg-green-600  px-6 py-2 rounded-xl">
