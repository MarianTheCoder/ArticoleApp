...
export default function ManoperaForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      formDataToSend.formFirst.cod == "" ||
      formDataToSend.formFirst.articol == "" ||
      formDataToSend.formFirst.unitate_masura == "" ||
      formDataToSend.formFirst.clasa == "" ||
      formDataToSend.formFirst.descriere_reteta == "" ||
      formDataToSend.formFirst.limba == ""
    ) {
          ...
          <div className="flex justify-center flex-col items-center text-black  ">
            <form onSubmit={handleSubmit} className="w-full p-4 pt-4 px-2 md:px-4 xl:px-6 rounded-xl">
              <div onClick={console.log(selectedDelete, selectedEdit, selectedDouble)} className={`grid text-sm ${"grid-cols-[auto_auto_auto_1fr_1fr_1fr_1fr_auto_auto]"} gap-2 md:gap-4 items-center`}>
                ...
                <div className="flex flex-col items-center ">
                  <label className=" font-medium text-black">Cod</label>
                  <input type="text" id="cod" name="cod" value={formData.cod} onChange={handleChange} maxLength={18} className="px-2 outline-none text-center py-2 max-w-40  rounded-lg shadow-sm " />
                </div>
                ...
                {formData.limba === "RO" ? (
                  <div className="flex flex-col items-center">
                    ...
                    <select id="clasa" name="clasa" value={formData.clasa} onChange={handleChange} className=" px-1 py-2 rounded-lg max-w-96 outline-none shadow-sm ">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <select id="clasa" name="clasa" value={formData.clasa} onChange={handleChange} className=" px-1 py-2 rounded-lg outline-none shadow-sm ">
                ...
                <div className="flex flex-col items-center">
                  <label className=" font-medium text-black">Articol</label>
                  <textarea
                    rows={3}
                    type="text"
                    id="articol"
                ...
                <div className="flex flex-col items-center">
                  <label className=" font-medium text-black">Articol FR</label>
                  <textarea
                    rows={3}
                    type="text"
                    id="articol_fr"
                ...
                <div className="flex flex-col items-center">
                  ...
                  <select id="unitate_masura" name="unitate_masura" value={formData.unitate_masura} onChange={handleChange} className=" px-2 py-2   rounded-lg outline-none shadow-sm ">
                ...
                {!selectedDelete && !selectedEdit && !selectedDouble ? (
                  <div className="flex gap-2 items-center ">
                    <button type="submit" className="bg-green-500 hover:bg-green-600 text-black mt-6 px-4 py-2 flex  items-center rounded-lg">
                      <FontAwesomeIcon icon={faPlus} className="pr-3" /> Încarcă
                    </button>
                  </div>
                ) : selectedDelete ? (
                  <div className="flex gap-2 items-center ">
                    <button onClick={(e) => deleteRow(e)} className="bg-red-500 hover:bg-red-600 text-black  mt-6 px-4 py-2 flex  items-center rounded-lg">
                      <FontAwesomeIcon icon={faCancel} className="pr-3" /> Șterge
                    </button>
                    <button onClick={(e) => cancelDelete(e)} className="bg-green-500 hover:bg-green-600 text-black  mt-6 px-4 py-2 flex  items-center rounded-lg">
                      Anulează
                    </button>
                  </div>
                ) : selectedDouble ? (
                  ...
                  <div className="flex gap-2 items-center ">
                    ...
                    <button onClick={(e) => cancelEdit(e)} className="bg-red-500 hover:bg-red-600 text-black  mt-6 px-4 py-2 flex  items-center rounded-lg">
                      {" "}
                      Anulează
                    </button>
                  </div>
