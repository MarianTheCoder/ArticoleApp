...
export default function SantiereAllPartsForExport({ mainOfertaPartID, ofertaId }) {
  ...
  const toggleDropdown = async (parentId) => {
    ...
    if (isAlreadyOpen) {
      ...
      try {
        ...
        const children = [...response.data.manopera, ...response.data.materiale, ...response.data.utilaje, ...response.data.transport].map((item) => ({
          ...item,
          parentId,
        }));
  ...
  const currentPartName = currentHeaderIndex != null ? allRows[currentHeaderIndex].original.partName : null;
            ...
            <div ref={scrollParentRef} className="  scrollbar-webkit overflow-auto  relative  ">
              {currentPartName && stickyTop > 0 && (
                <div className="sticky z-20" style={{ top: stickyTop }}>
                  <div className="bg-white border-b border-black h-12 flex items-center justify-center text-black text-lg font-semibold pointer-events-none">Lucrarea {currentPartName}</div>
                </div>
              )}
                  ...
                  {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id} className="bg-white text-black text-left  font-bold select-none">
                      {headerGroup.headers.map((header) => (
                        <th
                          key={header.id}
                          className={`relative border-b-2 border-r border-black text-base   bg-white p-2 py-4 ${header.column.id === "threeDots" ? "text-center" : ""} `}
                          style={{
