...
export default function SantiereAdd({
  ...
  const handleAcceptEdit = async () => {
    ...
    try {
      const res = await api.put(`/Santiere/updateSantierRetetaPrices`, {
        articolClient: currentArticolClient,
        santier_reteta_id: selectedEdit,
        reper_plan: currentReperPlan,
        detalii_aditionale: currentDetaliiAditionale,
        cantitate_reteta: currentCantitateReteta,
        updatedCosts: currentEditedCosts,
      });
        ...
        (prev) => {
          ...
          for (let i = 0; i < updated.length; i++) {
            ...
            if (item.parentId == selectedEdit) {
              const cost = currentEditedCosts[key] != undefined ? parseFloat(currentEditedCosts[key]) : parseFloat(item.cost);
          ...
          const parentIndex = updated.findIndex((item) => !item.whatIs && item.id == selectedEdit);
  ...
  const toggleDropdown = async (parentId, isEditClicked) => {
    ...
    if (isAlreadyOpen && !isEditClicked) {
      ...
      try {
        ...
        const children = [...response.data.manopera, ...response.data.materiale, ...response.data.utilaje, ...response.data.transport];
    ...
    try {
      ...
      if (response.data?.insertedId) {
        ...
        setDetailedCosts((prev) => {
          ...
          if (updated[parentId]?.[whatIs]?.[oldId]) {
  ...
  const replaceItemInDetailedCosts = (parentId, whatIs, removeId, addId, itemToAdd, cantitate, prevDetails) => {
  ...
  //
  //
  const parentIds = useMemo(() => (retete || []).filter((r) => !r.parentId).map((r) => r.id), [retete]);
  const allOpen = useMemo(() => parentIds.length > 0 && parentIds.every((id) => openDropdowns.has(id)), [parentIds, openDropdowns]);
  ...
  const parentIdsForDouble = useMemo(() => topLevelRows.map((r) => r.id), [topLevelRows]);
  ...
  const onMoveSelectClick = useCallback(
    (id, e) => {
  ...
  const handleDragEnd = async (event) => {
    ...
    const getBySortable = (sid) => topLevel.find((r) => r.sort_order.toString() === String(sid));
    ...
    const selectedSet = new Set((moveSelectedIds ?? []).filter((id) => topLevel.some((p) => p.id === id)));
    ...
    const newTop = [...rest.slice(0, insertIndex), ...selectedBlock, ...rest.slice(insertIndex)];
    ...
    () => [
      {
        ...
        header: (
          <div className="flex justify-center items-center h-full w-full">
            <FontAwesomeIcon onClick={toggleAllDropdowns} className="text-blue-500 hover:text-blue-600 cursor-pointer text-xl" icon={faArrowDownShortWide} />
          </div>
        ),
        cell: ({ row, getValue, cell }) => (
          <div
            onClick={() => row.original.id != selectedEdit && toggleDropdown(cell.row.original.id)}
            className="flex justify-center h-full  select-none w-full dropdown-container overflow-hidden cursor-pointer items-center"
          >
        ...
        cell: ({ row, getValue, cell }) =>
          row.original.whatIs == "Manopera" ? (
            <div className="w-full h-full flex justify-center items-center overflow-hidden">
              <FontAwesomeIcon className="text-green-500 h-[1.5rem] w-full " icon={faUser} />
            </div>
          ) : row.original.whatIs == "Material" ? (
            <div className="w-full h-full flex justify-center items-center overflow-hidden">
              <FontAwesomeIcon className="text-amber-500 h-[1.5rem] w-full " icon={faTrowelBricks} />
            </div>
          ) : row.original.whatIs == "Utilaj" ? (
            <div className="w-full h-full flex justify-center items-center overflow-hidden">
              <FontAwesomeIcon className="text-violet-500 h-[1.5rem] w-full  " icon={faTruck} />
            </div>
          ) : row.original.whatIs == "Transport" ? (
            <div className="w-full h-full flex justify-center items-center overflow-hidden">
              <FontAwesomeIcon className="text-pink-500 h-[1.5rem] w-full  " icon={faCar} />
            </div>
          ) : (
            <div onClick={() => console.log(retete)} className="w-full h-full flex justify-center items-center overflow-hidden ">
              <FontAwesomeIcon className="text-blue-500 h-[1.5rem]   " icon={faFolder} />
            </div>
          ),
      },
      {
        accessorKey: "detalii_aditionale",
        header: <div className="">{localReper1 || "Eroare"}</div>,
        size: 100,
        cell: ({ getValue, row }) => {
      ...
      },
      {
        ...
        header: <div className="">{localReper2 || "Eroare"}</div>,
        size: 100,
        cell: ({ getValue, row }) => {
          const isEditable = row.original.id === selectedEdit && !row.original.whatIs;
          return (
      ...
      { accessorKey: "furnizor", header: "Furnizor", size: 80 },
      {
        accessorKey: "cod",
        header: "Cod",
        cell: ({ getValue }) => <div className=" font-semibold">{getValue() ? getValue() : ""}</div>,
        size: 120,
      },
      {
        accessorKey: "clasa",
        header: "Clasă",
        cell: ({ getValue }) => <div className="">{getValue() ? getValue() : ""}</div>,
        size: 150,
      },
      {
          ...
          <div className="flex items-center w-[95%]  justify-between text-black ">
            ...
            <span className="flex items-center">
              ...
              <span
                ...
                className="ml-2 text-green-600 border-2 hover:text-green-500 hover:border-green-500 cursor-pointer border-green-600 rounded-full aspect-square min-w-[2rem] flex items-center justify-center"
              >
                {localLimba}
              </span>
            </span>
      ...
      },
      {
        ...
        size: 100,
        cell: ({ getValue, row }) =>
          getValue() ? <div className="w-full overflow-hidden text-sm">{row.original.whatIs == "Material" ? getValue() + " " + row.original.tip_material : getValue()}</div> : "Rețetă", // Display default value if the value is empty or undefined
      },
      ...
      },
      {
        ...
        header: "Pret Total",
        cell: ({ getValue, row }) => {
          return <span className="font-semibold">{(row.original.cost * row.original.cantitate).toFixed(2)}</span>;
        },
        size: 80,
      },
      {
        ...
        cell: ({ row }) =>
          row.original.whatIs == "Manopera" || row.original.whatIs == "Material" || row.original.whatIs == "Utilaj" || row.original.whatIs == "Transport" ? (
            ...
            <div className="absolute dropdown-container group w-full">
              ...
              {selectedDelete !== row.original.id && selectedEdit !== row.original.id && actualizareReteta !== row.original.id && (
                <div
                  className="absolute z-10 left-0 -translate-x-[60%] bg-white border shadow-lg rounded-md w-44 p-2
          ...
          <div className="p-2 pb-4 pt-5 xxxl:text-sm text-base scrollbar-webkit w-full overflow-hidden text-white h-full flex flex-col justify-between">
            <div ref={scrollParentRef} className="scrollbar-webkit h-full overflow-auto relative">
              <DndContext sensors={useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 5 } }))} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={topLevelRows.map((r) => r.sort_order.toString())} strategy={verticalListSortingStrategy}>
                    ...
                    {retete.length === 0 ? (
                      <tbody className="relative z-0">
                        <tr>
                          <td className="bg-[rgb(255,255,255,0.75)] border-black border-r border-b text-black h-10" colSpan={visibleColCount}>
                            <div className="flex justify-center items-center w-full text-base font-semibold h-full">Nimic Adăugat</div>
                          </td>
                        </tr>
                      </tbody>
                    ) : (
                      <tbody className="relative z-0">
                        ...
                        {virtualItems.map((vi) => {
                          ...
                          return row.original.whatIs === "Manopera" || row.original.whatIs === "Material" || row.original.whatIs === "Utilaj" || row.original.whatIs === "Transport" ? (
            ...
            <div className="mt-auto text-xs  gap-4 flex containerZ p-4  items-center">
              ...
              <table className="">
                <tbody>
                  <tr className="font-medium select-none">
                    ...
                    <td className="border border-black p-2  bg-blue-300 text-black">
                      Recapitulații
                      <input
                        maxLength={2}
                        value={recapitulatii}
                        ...
                        className="mx-2 max-w-10 font-normal outline-none text-center px-2 border border-black rounded-lg"
                      />
                      %
                    </td>
                    <td rowSpan={2} className="p-2 text-center">
                  ...
                  <tr className="font-semibold">
                    ...
                    <td className="border font-bold tracking-wide  border-black p-2 bg-white text-black">
                      {(
                        finalData.cheltuieliDirecte +
                        (recapitulatii / 100) * finalData.cheltuieliDirecte +
                        (TVA / 100) * (finalData.cheltuieliDirecte + (recapitulatii / 100) * finalData.cheltuieliDirecte)
                      ).toFixed(2)}
                    </td>
                  </tr>
                </tbody>
              ...
              <button
                onClick={() => setIsGenerareOpen(true)}
                className="bg-green-500 cursor-pointer flex gap-2 justify-center font-medium items-center p-2 mt-6 text-sm tracking-wide hover:bg-green-600 h-10 text-black rounded-lg flex-grow"
              >
...
function SortableRow({
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: row.original.sort_order.toString() });
            ...
            : isMoveSelected
              ...
              : row.original.id === selectedDelete
                ...
                : row.original.id === selectedEdit
                  ? "bg-green-300 "
                  : row.original.id === actualizareReteta
                    ? "bg-purple-300 "
  ...
  return row.original.articol === "Blank" ? (
    <tr onClick={handleRowClick} ref={setNodeRef} style={style} {...attributes} {...listeners} className={`text-black ${rowClass} relative`}>
      {/* Big "Blank" cell spanning all but the options column */}
      <td className="border-b border-r border-black bg-white p-1 px-3" colSpan={otherCellsCount}>
      ...
      {optionsCell && (
        <td key={optionsCell.id} className="border-b border-r break-words max-w-72 relative border-black p-1 px-3" style={optionsCell.column.columnDef.meta?.style}>
          <div className="h-full w-full overflow-hidden">
    ...
    <tr onClick={handleRowClick} ref={setNodeRef} style={style} {...attributes} {...listeners} className={rowClass}>
      {cells.map((cell) => (
        <td key={cell.id} className="border-b border-r break-words max-w-72 relative border-black p-1 px-3" style={cell.column.columnDef.meta?.style}>
          <div className="h-full w-full overflow-hidden">
            <div className="max-h-9 h-9 w-full grid grid-cols-1 items-center break-words whitespace-pre-line overflow-auto scrollbar-webkit">
              {flexRender(cell.column.columnDef.cell, cell.getContext())}
            </div>
