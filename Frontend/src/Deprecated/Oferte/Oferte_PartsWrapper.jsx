...
export default function Oferte_PartsWrapper({ ofertaId }) {
  ...
  const fetchFurnizori = async (type) => {
    ...
    try {
      ...
      console.log("❌ Eroare la fetch furnizori:", err);
      setFurnizori([]);
    } finally {
  ...
  const acceptActualizare = async () => {
    try {
      ...
      console.log("❌ Eroare la actualizarea rețetelor:", error);
    } finally {
  ...
  const saveRepere = async () => {
    try {
      ...
      console.log("❌ Eroare la salvare repere:", error);
    }
  };
  ...
  const duplicateSelectedRows = async () => {
    ...
    try {
      ...
      console.log("❌ Eroare la dublarea rețetelor:", err);
    }
  };
    ...
    <div className="relative h-full w-full grid overflow-hidden p-4 grid-rows-[auto_1fr] ">
      <div className=" px-8 p-3 bg-[#26415f] rounded-xl flex w-full justify-between">
        <div className="flex gap-4  items-center">
          ...
          {editRepere && (
            <div className="flex gap-4 items-center">
              <input type="text" placeholder="Reper 1" value={reper1} onChange={(e) => setReper1(e.target.value)} className="py-2 w-64 px-2 text-black rounded-lg outline-none shadow-sm" />
              <input type="text" placeholder="Reper 2" value={reper2} onChange={(e) => setReper2(e.target.value)} className="py-2 w-64 px-2 text-black rounded-lg outline-none shadow-sm" />
          ...
          {actualizeazaRetete && (
            <div className="flex gap-4 items-center">
              ...
              <button onClick={acceptActualizare} className="bg-purple-500 flex  items-center justify-center gap-2 text-white  hover:bg-purple-600  px-4 py-3 rounded-xl">
                <FontAwesomeIcon className="" icon={faRotate} />
                <span className="leading-none">Actualizează!</span>
              </button>
          ...
          {selectedPartId != "" && dubleazaRetete && (
            <div className="flex gap-4">
              ...
              <div className="flex items-center justify-center">
                <button onClick={() => setChooseAnchor(true)} className="bg-blue-500 text-white px-4 py-3 rounded-xl hover:bg-blue-600">
                  Alege poziția
                </button>
              </div>
              <div className="flex items-center justify-center">
                <button onClick={duplicateSelectedRows} className="bg-amber-500 flex items-center justify-center gap-2 text-white hover:bg-amber-600 px-4 py-3 rounded-xl">
                  ...
                  <span className="leading-none">Dublează</span>
                </button>
              </div>
              {/* detalii_aditionale */}
            ...
            <div className="flex gap-4">
              ...
              <div className="flex items-center justify-center">
                <button onClick={handleAcceptDeleteRetete} className="bg-rose-500 flex items-center justify-center gap-2 text-white hover:bg-rose-600 px-4 py-3 rounded-xl">
                  ...
                  <span className="leading-none">Sterge</span>
                </button>
              </div>
          ...
          {selectedPartId != "" && showFurnizori && (
            <div className="flex gap-4 items-center">
              ...
              <div className="flex gap-2">
                <button onClick={() => changeFurnizoriType("materiale")} className={`px-3 py-2 rounded-lg ${furnizoriType === "materiale" ? "bg-cyan-600 text-white" : "bg-white text-black"}`}>
                  Materiale
                </button>
                <button onClick={() => changeFurnizoriType("utilaje")} className={`px-3 py-2 rounded-lg ${furnizoriType === "utilaje" ? "bg-cyan-600 text-white" : "bg-white text-black"}`}>
                  Utilaje
                </button>
              </div>

              <div onClick={() => console.log(furnizori)} className="text-white">
                Furnizor:
              </div>
              <select
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    await api.post(
                      `/Santiere/aplicaFurnizorPeToate/${selectedPartId}`,
                      {
                    ...
                    console.log("❌ Eroare aplicare furnizor:", e);
                  }
                }}
              >
                Aplică pe toate
          ...
          {!deleteOfertaPart && !editOfertaPart && !editRepere && !actualizeazaRetete && !dubleazaRetete && !showFurnizori && !deleteRetete ? (
            <button onClick={() => handleAddOfertaPart()} className="bg-green-500 flex  items-center justify-center gap-2 text-white  hover:bg-green-600  px-4 py-2 rounded-xl">
              <FontAwesomeIcon className="" icon={faPlus} />
              Adaugă Lucrare
            </button>
          ) : deleteOfertaPart ? (
            <div className="flex gap-4">
              <button onClick={() => setDeleteOfertaPart(false)} className="bg-green-500 flex  items-center justify-center gap-2 text-white  hover:bg-green-600  px-4 py-2 rounded-xl">
                Anluează
              </button>
              <button onClick={() => deleteLucrare()} className="bg-red-500 flex  items-center justify-center gap-2 text-white  hover:bg-red-600  px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faX} />
                <span className="leading-none">Șterge Lucrarea</span>
              </button>
            </div>
          ) : editOfertaPart ? (
            <div className="flex gap-4">
              ...
              <button onClick={() => handleCancel()} className="bg-red-500 flex  items-center justify-center gap-2 text-white  hover:bg-red-600  px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faX} />
                <span className="leading-none">Anulează</span>
              </button>
            </div>
          ...
          {addOfertaPartInput && !editOfertaPart && !deleteOfertaPart ? (
            <button onClick={() => handleCancel()} className="bg-red-500 flex  items-center justify-center gap-2 text-white  hover:bg-red-600  px-4 py-2 rounded-xl">
              <FontAwesomeIcon className="" icon={faCancel} />
              Anluează
            </button>
          ) : (
            ""
        ...
        <div className="flex gap-4">
          {selectedPartId != "" && !deleteOfertaPart && !editOfertaPart && !editRepere && !actualizeazaRetete && !dubleazaRetete && !showFurnizori && !deleteRetete ? (
            <div className="flex gap-4">
              <button onClick={() => handleFurnizori()} className="bg-cyan-600 flex items-center justify-center gap-2 text-white hover:bg-cyan-700 px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faScrewdriverWrench} />
                <span className="leading-none">Furnizori</span>
              </button>
              <button onClick={() => handleDubleazaRetete()} className="bg-amber-500 flex  items-center justify-center gap-2 text-white  hover:bg-amber-600  px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faCopy} />
                <span className="leading-none">Dubleazǎ Rețetele</span>
              </button>
              <button onClick={() => handleActualizeazaRetete()} className="bg-purple-500 flex  items-center justify-center gap-2 text-white  hover:bg-purple-600  px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faRotate} />
                <span className="leading-none">Actualizează Rețetele</span>
              </button>
              <button onClick={() => handleEditRepere()} className="bg-blue-500 flex  items-center justify-center gap-2 text-white  hover:bg-blue-600  px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faEdit} />
                <span className="leading-none">Editează Reperele</span>
              </button>
              <button onClick={() => handleEdit()} className="bg-green-500 flex  items-center justify-center gap-2  text-white  hover:bg-green-600  px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faEdit} />
                Editează
              </button>
              <button onClick={() => handleDeleteRetete()} className="bg-rose-500 flex  items-center justify-center gap-2 text-white  hover:bg-rose-600  px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faTrash} />
                <span className="leading-none">Șterge Rețetele</span>
              </button>
              <button onClick={() => handleDelete()} className="bg-red-800 flex  items-center justify-center gap-2 text-white  hover:bg-red-900  px-4 py-2 rounded-xl">
                <FontAwesomeIcon className="" icon={faX} />
                <span className="leading-none">Șterge Lucrarea</span>
              </button>
            </div>
