...
import { faArrowDown, faArrowDownAZ, faArrowUpAZ, faCancel, faCar, faChevronDown, faChevronRight, faCopy, faEllipsis, faFolder, faL, faLanguage, faPenToSquare, faPerson, faPlus, faTrashCan, faTrowelBricks, faTruck, faUser, faX } from '@fortawesome/free-solid-svg-icons';
...
export default function SantiereAddReteteTableAbsolute({
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            header: "Limba",
            cell: ({ getValue, row }) =>
                <div className='w-full flex justify-center  font-semibold'> {getValue()}</div>, // Display default value if the value is empty or undefined
            size: 20
        },
        ...
        },
        {
            ...
            header: 'Tip',
            size: 100,
            cell: ({ getValue, row }) => getValue() ? <div onClick={() => console.log(retete)} className='w-full'>{row.original.whatIs == "Material" ? getValue() + " " + row.original.tip_material : getValue()}</div> : 'Rețetă', // Display default value if the value is empty or undefined
        },
        ...
        { accessorKey: "cantitate", header: "Cantitate", size: 70 },
        {
            ...
            cell: ({ getValue, row }) => {
                ...
                return value ? <div className='w-full font-semibold text-center'>{parseFloat(value).toFixed(3)}</div> : row.original.cost ? <div className='w-full font-medium text-center'>{row.original.cost}</div> : <div className='w-full font-medium text-center'>0.000</div>;
            },
            size: 100
        },
        {
