...
export default function SantiereAddReteteTableAbsolute({ setIsPopupOpen, fetchParentRetete, mainOfertaPartID, reper1, reper2 }) {
  const { idSantier, limbaUser } = useParams();

  const [selectedReteteIds, setSelectedReteteIds] = useState([]);
  ...
  const fetchManopere = async () => {
    try {
      if (filters.cod.trim().length >= 2 || filters.articol.trim().length >= 2) {
  ...
  const toggleDropdown = async (e, reteta_id) => {
    e.stopPropagation();
    const isAlreadyOpen = openDropdowns.has(reteta_id);
    // console.log(reteta_id, isAlreadyOpen);

    if (isAlreadyOpen) {
      // Remove children from retete and mark dropdown as closed
      setRetete((prev) => prev.filter((item) => item.reteta_id != reteta_id));
      setOpenDropdowns((prev) => {
        const newSet = new Set(prev);
        newSet.delete(reteta_id);
        return newSet;
      });
    } else {
      try {
        const response = await api.get(`/Retete/getSpecificReteta/${reteta_id}`);
        const children = [...response.data.manopera, ...response.data.materiale, ...response.data.utilaje, ...response.data.transport];

        setRetete((prev) => {
          const index = prev.findIndex((item) => item.id === reteta_id);
  ...
  const toggleReteteChangeLanguage = (id) => {
    setSelectedReteteIds((prev) => {
  ...
  const columns = useMemo(
    () => [
        ...
        cell: ({ row, getValue, cell }) =>
          row.original.whatIs == "Manopera" ? (
            <div className="w-full h-full flex justify-center items-center overflow-hidden">
              <FontAwesomeIcon className="text-green-500 h-[2rem] w-full " icon={faUser} />
            </div>
          ) : row.original.whatIs == "Material" ? (
            <div className="w-full h-full flex justify-center items-center overflow-hidden">
              <FontAwesomeIcon className="text-amber-500 h-[2rem] w-full " icon={faTrowelBricks} />
            </div>
          ) : row.original.whatIs == "Utilaj" ? (
            <div className="w-full h-full flex justify-center items-center overflow-hidden">
              <FontAwesomeIcon className="text-violet-500 h-[2rem] w-full  " icon={faTruck} />
            </div>
          ) : row.original.whatIs == "Transport" ? (
            <div className="w-full h-full flex justify-center items-center overflow-hidden">
              <FontAwesomeIcon className="text-pink-500 h-[2rem] w-full  " icon={faCar} />
            </div>
          ) : (
            <div className="w-full h-full flex justify-center items-center overflow-hidden ">
              <FontAwesomeIcon
                className={`${row.original?.has_manopera > 0 || row.original?.has_materiale > 0 || row.original?.has_transport > 0 || row.original?.has_utilaje > 0 ? " text-blue-500" : "text-gray-400"} h-[2rem] `}
                icon={faFolder}
              />
            </div>
          ),
      },
      {
        accessorKey: "limba",
        header: "Limba",
        cell: ({ getValue, row }) => <div className="w-full flex justify-center  font-semibold"> {getValue()}</div>, // Display default value if the value is empty or undefined
        size: 20,
      },
      { accessorKey: "cod", header: "Cod", size: 100 },
      { accessorKey: "clasa", header: "Clasă", size: 130 },
      {
        accessorKey: "articol",
        header: "Articol",
        cell: ({ getValue, row }) =>
          selectedReteteIds.includes(row.original.id) ? (
            <div className="">
              <OverflowPopover text={row.original.articol_fr || "..."} maxLines={2} />
            </div>
          ) : (
            <OverflowPopover text={getValue()} maxLines={2} />
          ),
        size: 350,
      },
      {
        ...
        cell: ({ getValue, row }) =>
          selectedReteteIds.includes(row.original.id) ? (
            <div className="">
              <OverflowPopover text={row.original.descriere_fr || "..."} maxLines={2} />
            </div>
          ) : (
            <OverflowPopover text={getValue()} maxLines={2} />
          ),
      ...
      },
      {
        ...
        cell: ({ getValue, row }) =>
          getValue() ? (
            <div onClick={() => console.log(retete)} className="w-full">
              {row.original.whatIs == "Material" ? getValue() + " " + row.original.tip_material : getValue()}
            </div>
          ) : (
            "Rețetă"
      ...
      { accessorKey: "unitate_masura", header: "Unitate", size: 20 },
      {
        ...
        cell: ({ getValue }) =>
          getValue() ? (
            <div className="flex justify-center p-1 overflow-hidden w-full h-full items-center">
              <img
      ...
      { accessorKey: "cantitate", header: "Cantitate", size: 70 },
      {
        ...
        cell: ({ getValue, row }) => {
          ...
          return value ? (
            <div className="w-full font-semibold text-center">{parseFloat(value).toFixed(3)}</div>
          ) : row.original.cost ? (
            <div className="w-full font-medium text-center">{row.original.cost}</div>
          ) : (
            <div className="w-full font-medium text-center">0.000</div>
          );
        },
        size: 100,
      },
      {
        ...
        header: (
          <div className="flex w-full text-sm   justify-center">
            <div onClick={() => translateAll()} className="bg-blue-500 rounded-xl px-2 w-full hover:bg-blue-600 hover:cursor-pointer flex gap-2 p-2 items-center justify-center">
        ...
        cell: ({ row }) => (
          <div className="w-full relative overflow-hidden flex ">
            <div className="text-3xl relative w-full py-2 select-none items-center justify-evenly gap-2 flex">
              <FontAwesomeIcon onClick={() => toggleReteteChangeLanguage(row.original.id)} className=" text-blue-500 hover:text-blue-600 cursor-pointer" icon={faLanguage} />
            </div>
          </div>
        ),
        size: 50,
    ...
    [retete, ascendent, selectedReteteIds],
  );
        ...
        <div className=" containerZ flex flex-col h-full w-full overflow-hidden">
          <div className="grid font-medium grid-cols-[auto_auto_1fr_3fr] gap-4 p-4 pt-2 text-black containerWhiter w-full">
            <div className="flex flex-col items-center">
              ...
              <input disabled={true} value={limbaUser} className={`px-2 w-24 bg-white  text-center py-2 rounded-lg shadow-lg outline-gray-300 `} />
            </div>
            {filters.limba == "RO" ? (
              <div className="flex flex-col items-center">
                ...
                <select id="clasa" name="clasa" value={filters.clasa} onChange={handleChangeFilters} className=" px-1 py-2  text-center rounded-lg outline-none shadow-sm ">
              ...
              <div className="flex flex-col items-center">
                ...
                <select id="clasa" name="clasa" value={filters.clasa} onChange={handleChangeFilters} className=" px-1 py-2 rounded-lg outline-none shadow-sm ">
                  <option value="">Toate</option>
            ...
            <div className="flex w-full flex-col items-center ">
              <label className=" text-black">Cod</label>
              <input
                type="text"
                id="cod"
                name="cod"
            ...
            <div className="flex flex-col w-full items-center ">
              <label className=" text-black">Articol</label>
              <input
                type="text"
                id="articol"
                name="articol"
          ...
          <div className=" w-full text-sm  flex flex-col h-full justify-between gap-4 overflow-hidden p-4 ">
                    ...
                    <tbody className="relative z-0">
                      <tr>
                        <td className="bg-white text-black h-12" colSpan={13}>
                          <div className=" flex justify-center items-center w-full text-base font-semibold h-full">Introdu minim 2 caractere</div>
                        </td>
                      </tr>
                    </tbody>
                  ) : retete.length == 0 ? (
                    ...
                    <tbody className=" relative z-0">
                      {table.getRowModel().rows.map((row, index, rows) =>
                        row.original.whatIs == "Manopera" || row.original.whatIs == "Material" || row.original.whatIs == "Utilaj" || row.original.whatIs == "Transport" ? (
                          ...
                          <React.Fragment key={row.id}>
                            <tr
                              onClick={() => setSelectedRetete((prev) => (prev?.original.id == row.original.id ? null : row))}
            ...
            <div className="flex gap-4 w-full ">
              <div className="flex gap-3 items-center ">
                <div className="flex flex-col items-center h-full">
                  <label className=" font-medium">Cantitate</label>
                  <input
                    type="text"
                    id="cantitate"
                    name="cantitate"
                ...
                <div className="flex flex-col items-center h-full">
                  <label htmlFor="reperPlan" className="font-medium whitespace-nowrap">
                    Denumire Client
                  </label>
                  <textarea
                    rows={3}
              ...
              <div className="w-full flex gap-3 pt-6 ">
                <button
                  ...
                  className="bg-green-500  dropdown-container flex  items-center justify-center gap-2 text-black flex-grow hover:bg-green-600  px-6 py-2 h-12 rounded-xl"
                >
                  <FontAwesomeIcon className="" icon={faPlus} />
                  <span className="leading-none">Adaugă Rețetă</span>
                </button>
                <button onClick={() => setIsPopupOpen(false)} className="bg-red-500 dropdown-container flex  items-center justify-center gap-2 text-black  hover:bg-red-600 h-12  px-6 py-2 rounded-xl">
