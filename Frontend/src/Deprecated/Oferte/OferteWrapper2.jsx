    ...
    return (
        loading ?
            ...
            <div className='relative h-screen w-full  overflow-hidden flex items-end justify-center'>
                <div className=" w-[99%] h-[96%] relative flex justify-center rounded-lg ">
                    <div className="absolute flex -top-7 left-20 -space-x-2 select-none">
                            ...
                            >
                                {editText != item.id ?
                                    <p
                                        ...
                                        className={`input_toEdit relative overflow-hidden  whitespace-nowrap  text-ellipsis hover:-translate-y-1 cursor-pointer   transition-all duration-[150ms]   text-white px-6 p-2 xxxl:px-4 rounded-tr-[4rem] rounded-tl-2xl  ${selectedButton == item.id && index != oferte.length - 1 ? "bg-[#143458] -translate-y-1 shadow-[8px_8px_15px_rgba(0,0,0,1)] " : selectedButton == item.id && index == oferte.length - 1 ? "bg-[#143458] -translate-y-1 shadow-[4px_4px_10px_rgba(0,0,0,1)]" : "bg-[#265f5a] shadow-[4px_4px_10px_rgba(0,0,0,1)]"}`}>{item.name}</p>
                                    :
                                    <React.Fragment >
                                        <input
                                            style={{
                                            ...
                                            className={`input_toEdit w-full relative overflow-hidden  whitespace-nowrap  text-ellipsis hover:-translate-y-1 cursor-pointer   transition-all duration-[150ms]    text-white px-6 p-3 xxxl:px-4 rounded-tr-[4rem] rounded-tl-2xl  ${selectedButton == item.id ? "bg-[#143458] -translate-y-1 shadow-[8px_8px_15px_rgba(0,0,0,1)] " : "bg-[#265f5a] shadow-[4px_4px_10px_rgba(0,0,0,1)]"}`}
                                        />
                        ...
                        {editText == null && <button onClick={() => addAutomaticOferta()} className={` relative  hover:bg-green-600  z-40 text-white px-4 p-2 rounded-tr-3xl rounded-tl-xl bg-green-500 shadow-[4px_4px_10px_rgba(0,0,0,1)] `}><FontAwesomeIcon icon={faPlus} /></button>}
                    </div>
