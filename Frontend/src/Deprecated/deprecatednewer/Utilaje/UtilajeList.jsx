...
import UtilajeSubList from "./UtilajeSubList"; // <-- Import actualizat
import NoImage from "@/assets/no-image-icon.png";
import ImagePreviewTooltip from "@/components/ui/ImagePreviewTooltip";

// --- VIRTUALIZATION COMPONENTS ---
const componentsUtilaje = {
  ...
  TableRow: (props) => {
    ...
    const parent = props.context?.utilaje?.[index];
...
const UtilajeList = memo(({ utilaje = [], visibleColumns, setDraft, setOpen, handleDeleteClick, displayLang = "RO", handleDuplicateClick }) => {
  ...
  useEffect(() => {
    if (selectedParent) {
      const freshParent = utilaje.find((p) => p.id === selectedParent.id);
  ...
  return (
    <>
                ...
                {showCol("poza") && (
                  <TableCell onContextMenu={(e) => e.stopPropagation()} className="text-center px-4 py-2 w-[6rem] max-w-[6rem]">
                    <ImagePreviewTooltip
                      ...
                      ringColor="hover:ring-rose-600" // Culoare adaptată pentru utilaje
      ...
      <UtilajeSubList open={subDialogOpen} setOpen={setSubDialogOpen} parentItem={selectedParent} />
    </>
  );
});

export default UtilajeList;
