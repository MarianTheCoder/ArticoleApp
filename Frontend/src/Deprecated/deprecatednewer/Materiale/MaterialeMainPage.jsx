...
// --- COMPONENTE COPIL ---
import MaterialeFilters from "./MaterialeFilters";
import MaterialeList from "./MaterialeList";
import MaterialeDefDialog from "./MaterialeDefDialog";
import SpinnerElement from "@/MainElements/SpinnerElement";
import DeleteDialog from "@/components/ui/delete-dialog";
import { toast } from "sonner";

export default function MaterialeMainPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState({
    poza: true, // Adăugată coloana de poză
    limba: true,
    variante: true,
    cod: true,
    denumire: true,
  ...
  // --- DATA FETCHING (GET) DIN BACKEND (Atenție: resursa este 'material') ---
  const { data, isFetching } = useCatalog("material", {
  ...
  const materialeList = data?.items || [];
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteDefinitie({
        id: itemToDelete.id,
        tip_resursa: "material",
      });
      toast.success("Materialul a fost șters cu succes.");
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch (error) {
      toast.error(error?.response?.data?.message || "Eroare la ștergerea materialului.");
    } finally {
      hide();
    }
  };
  ...
  return (
    <div className="h-full w-full flex justify-center overflow-hidden items-center">
      <div className="w-[95%] h-[95%] flex flex-col p-4 gap-4 overflow-hidden bg-background relative rounded-lg">
        ...
        <div className="flex-1 w-full bg-card rounded-lg overflow-hidden relative shadow-base border border-border flex flex-col">
          <div className="flex-1 p-5 overflow-hidden flex flex-col relative">
            {materialeList.length > 0 ? (
              <MaterialeList
                materiale={materialeList}
                visibleColumns={visibleColumns}
                setDraft={setDraft}
              ...
              <div className="flex-1 w-full flex justify-center items-center rounded-lg border border-border bg-muted/10">
                <span className="text-xl text-muted-foreground italic">
                  {searchDebounced.trim() === "" && advancedFiltersDebounced.cod === ""
                    ? "Nu am găsit niciun material conform criteriilor de căutare."
                    : "Nu am găsit niciun material conform criteriilor de căutare."}
                </span>
              </div>
            )}
          ...
          <div className="shrink-0 border-t border-border bg-muted/10 p-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <Button variant="default" size="lg" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="bg-amber-600 hover:bg-amber-700">
                <FontAwesomeIcon icon={faChevronLeft} className="text-sm " /> Înapoi
              </Button>
              <span className="text-base font-semibold text-foreground">
                Pagina {page} / {totalPages || 1}
              </span>
              <Button variant="default" size="lg" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= (totalPages || 1)} className="bg-amber-600 hover:bg-amber-700">
      ...
      <DeleteDialog
        ...
        description={`Ești sigur că vrei să ștergi materialul "${itemToDelete?.denumire}"? Această acțiune va șterge și variantele asociate.`}
        onSubmit={handleConfirmDelete}
        useCode={false}
      />
      <MaterialeDefDialog open={openAddDef} setOpen={setOpenAddDef} mode={draft ? "edit" : "add"} initialData={draft} />
    </div>
  );
}
