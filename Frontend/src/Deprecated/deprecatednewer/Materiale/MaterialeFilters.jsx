...
import {
  faMagnifyingGlass,
  faPlus,
  faBoxOpen, // Schimbăm iconița pentru Materiale
  faLanguage,
  faColumns,
  faFilter,
  faChevronDown,
...
export default function MaterialeFilters({ search, setSearch, totalItems, onAddClick, displayLang, onDisplayLangToggle, visibleColumns, toggleCol, advancedFilters, setAdvancedFilters }) {
    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen} className="w-full bg-card flex flex-col gap-4 rounded-lg px-6 py-4 shadow-sm border border-border shrink-0 z-10">
      <div className="flex flex-wrap xl:flex-nowrap items-center justify-between gap-4 w-full">
        <div className="flex items-center gap-3 w-full xl:w-auto">
          <Button variant="default" size="lg" className="gap-2 bg-amber-600 hover:bg-amber-700" onClick={onAddClick}>
            <FontAwesomeIcon icon={faPlus} className="text-base" />
            <span className="tracking-wide">Adaugă Material</span>
          </Button>
        ...
        <div className="relative justify-end w-full gap-2 xl:gap-4 flex flex-wrap xl:flex-nowrap items-center">
          ...
          <div className="flex items-center gap-2 text-muted-foreground font-medium shrink-0">
            <FontAwesomeIcon className="text-2xl text-amber-600" icon={faScrewdriverWrench} />
            <span>{totalItems} Materiale</span>
          </div>
        </div>
      </div>
