...
import { faPlus, faPenToSquare, faTags, faTrash, faCopy, faPerson, faLanguage, faColumns } from "@fortawesome/free-solid-svg-icons";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { useDeleteCatalogSubDef } from "@/hooks/Database/useCatalog";

import ManoperaSubDefDialog from "./ManoperaSubDefDialog";
import OverflowTooltip from "@/components/ui/OverflowTooltip";
import photoAPI from "@/api/photoAPI"; // <-- Importul pentru calea pozelor
import { useLoading } from "@/context/LoadingContext";
import DeleteDialog from "@/components/ui/delete-dialog";
import { toast } from "sonner";

export default function ManoperaSubList({ open, setOpen, parentItem }) {
  ...
  // --- STATE PENTRU LIMBĂ UI ---
  const [displayLang, setDisplayLang] = useState("RO");

  // --- STATE PENTRU COLOANE VIZIBILE ---
  ...
  const handleDuplicateSub = (sub) => {
    setSubDraft({
      ...sub,
      cod_specific: "", // Lăsăm gol să îl pună userul
      isDuplicate: true, // Flag pentru a ști că suntem în mod duplicare
    });
    setDefOpen(true);
  };
  ...
  console.log("Subcategorii pentru manopera", parentItem.id, ":", subcategorii); // Debug log pentru subcategorii
  return (
    <>
        ...
        <DialogContent className="lg:max-w-[95rem] max-h-[85vh] h-2/3 flex flex-col p-0">
          <DialogHeader className="px-6 py-4 border-b bg-muted flex flex-row items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-3 mr-10">
              ...
              <Button onClick={handleAddSub} className="gap-2 bg-indigo-600 hover:bg-indigo-700 h-10">
          ...
          <div className="p-6 overflow-hidden flex-1 flex flex-col">
            {subcategorii.length > 0 ? (
                ...
                <Table className="min-w-full table-fixed caption-bottom text-left border-collapse">
                  ...
                  <TableBody>
                    {subcategorii.map((sub) => {
                      // Determinăm descrierea în funcție de limba selectată
              ...
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground rounded-xl">
                ...
                <p className="text-lg">Nu există variante definite pentru această manoperă.</p>
              </div>
            )}
          </div>
        </DialogContent>
      ...
      <DeleteDialog
        ...
        useCode={false}
      />
      <ManoperaSubDefDialog open={defOpen} setOpen={setDefOpen} mode={subDraft ? "edit" : "add"} initialData={subDraft} definitieId={parentItem.id} />
    </>
  );
}
