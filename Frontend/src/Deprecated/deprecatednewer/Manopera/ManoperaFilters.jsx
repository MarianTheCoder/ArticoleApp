...
export default function ManoperaFilters({ search, setSearch, totalItems, onAddClick, displayLang, onDisplayLangToggle, visibleColumns, toggleCol, advancedFilters, setAdvancedFilters }) {
  ...
  // Detectăm ce filtre sunt active pentru a randa tag-urile
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen} className="w-full bg-card flex flex-col gap-4 rounded-lg px-6 py-4 shadow-sm border border-border shrink-0 z-10">
      {/* ─── ROW 1: ACȚIUNI, SORTARE ȘI CĂUTARE GLOBALĂ ─── */}
      <div className="flex flex-wrap xl:flex-nowrap items-center justify-between gap-4 w-full">
        <div className="flex items-center gap-3 w-full xl:w-auto">
          <Button variant="default" size="lg" className=" bg-indigo-600 hover:bg-indigo-700 gap-2" onClick={onAddClick}>
            <FontAwesomeIcon icon={faPlus} className="text-base" />
            <span className="tracking-wide">Adaugă Manoperă</span>
          </Button>
        ...
        <div className="relative justify-end w-full gap-2 xl:gap-4 flex flex-wrap xl:flex-nowrap items-center">
          ...
          <div className="flex items-center gap-1 text-muted-foreground font-medium shrink-0">
            <FontAwesomeIcon className="text-2xl text-indigo-500" icon={faPerson} />
            <span>{totalItems} Manopere</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2 items-center px-1">
          ...
          {activeFilters.map(([key, value]) => (
            <Badge key={key} variant="outline" className="gap-2 px-2 bg-muted text-sm h-8 font-medium border">
              <span className="">{key}:</span> {value == "1" ? "Da" : value == "0" ? "Nu" : value} {/* Tratăm cazul pentru filtrele boolean ca "Da"/"Nu" */}
      ...
      {/* ─── ROW 2: FILTRE AVANSATE (COLLAPSIBLE) ─── */}
