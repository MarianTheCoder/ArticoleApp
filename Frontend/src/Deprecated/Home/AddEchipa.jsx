...
export default function Echipa() {
  ...
  useEffect(() => {
    const fetchTeam = async () => {
      try {
        ...
        console.log("Eroare la preluarea echipei:", error);
      }
    };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.log("Eroare la ștergerea membrului:", error);
  ...
  const handleSaveEdit = async (e) => {
    ...
    if (editingMember) {
      ...
      try {
        ...
        setTeam((prevTeam) => prevTeam.map((member) => (member.id === editingMember.id ? updatedMember : member)));
      ...
      <div className="containerZ w-2/3 h-90h relative flex flex-col items-center rounded-lg">
        <div className="w-full relative h-full gap-2 rounded-xl flex flex-col overflow-hidden p-5 py-8">
          <div className="min-h-screen bg-[#000043] flex flex-col items-center justify p-6">
            <h2 className="text-3xl font-bold mb-6 text-white">{editingMember ? "Editează Membru" : "Adaugă un Membru în Echipa"}</h2>
            ...
            >
              ...
              <div className="max-h-36">
                <input type="text" placeholder="Nume" className="text-black w-full p-3 border rounded max-h-36" value={name} onChange={(e) => setName(e.target.value)} required />
              </div>
              <div className="">
                <input type="text" placeholder="Rol" className="text-black w-full p-3 border rounded" value={role} onChange={(e) => setRole(e.target.value)} required />
              </div>
              <div className="max-h-36">
                <textarea
            ...
            <div className="w-full max-w-4xl">
              {team.map((member) => (
                <div key={member.id} className="flex items-center justify-between bg-white p-6 mb-4 rounded-lg shadow-lg">
                  ...
                  <div>
                    <button onClick={() => handleEdit(member)} className="p-2 bg-yellow-600 text-white rounded hover:bg-yellow-700 mr-2">
                      Editează
                    </button>
