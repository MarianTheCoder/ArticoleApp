...
export default function ManoperaTable({
  ...
  const delPreviewMaterial = (id) => {
    if (materiale) {
      ...
      const materialIndex = updatedmateriale.findIndex((item) => item.id === id && item.cod_definitie != null); // sa fie diferit de materiale/material etc
      const addButtonIndex = updatedmateriale.findIndex((item) => item.id === `addButton-${id}`);
      // console.log("reteIDX ", materialIndex, "btnIDX ", addButtonIndex);
      if (addButtonIndex !== -1 && materialIndex !== -1 && addButtonIndex > materialIndex) {
        updatedmateriale.splice(materialIndex + 1, addButtonIndex - materialIndex);
  ...
  const fetchPreviewMaterial = async (id, index, materialeParam, row) => {
    try {
      const response = await api.get(`/Materiale/api/getSpecificMaterial/${id}`);
      ...
      const updatedMateriale = materialeParam ? [...materialeParam] : [...materiale];
  ...
  const handleSubmit = async (e, row, parentId, data = null, file = null) => {
    ...
    try {
      const parent = materiale.find((item) => item.id == parentId && item.cod_definitie != null);
      if (!parent) return;
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "tip_material", header: "Tip", size: 80 },
      {
        ...
        header: (
          <div onClick={() => console.log(materiale)} className="flex items-center w-[95%] justify-between text-black ">
            <span>Furnizor</span>
          </div>
        ),
        cell: ({ getValue, row }) => {
    ...
    [selectedDelete, selectedDeleteInterior, selectedEdit, selectedEditInterior, selectedDouble, ascendent, open, materiale, selectedFileInterior, previewInterior, localLimba],
  );
            ...
            <table className="w-full border-separate text-sm border-spacing-0 ">
              <thead className="top-0 w-full sticky  z-10 ">
                <tr className="text-black">
                  ...
                  <th className="border-b border-r bg-white border-black">
                    <select id="clasa_material" name="clasa_material" value={filters.clasa_material} onChange={handleInputChange} className=" p-2 w-full cursor-pointer outline-none py-3">
                  ...
                  <th className="border-b bg-white border-r border-black">
                    <select id="tip_material" name="tip_material" value={filters.tip_material} onChange={handleInputChange} className="p-2 w-full cursor-pointer outline-none py-3 ">
                  ...
                  <th className="border-b bg-white border-r border-black">
                    <input type="text" name="furnizor" value={filters.furnizor} onChange={handleInputChange} className="p-2 w-full outline-none py-3" placeholder="Filtru Furnizor " />
                  </th>

                  <th className="border-b bg-white border-r border-black">
                    <input type="text" name="cod_definitie" value={filters.cod_definitie} onChange={handleInputChange} className="p-2 w-full outline-none py-3" placeholder="Filtru Cod " />
                  </th>
                  <th className="border-b bg-white border-r border-black">
                    <input type="text" name="denumire" value={filters.denumire} onChange={handleInputChange} className="p-2 w-full outline-none  py-3" placeholder="Filtru Denumire" />
                  </th>
                  <th className="bg-white border-b border-r border-black">
                    <input type="text" name="descriere" value={filters.descriere} onChange={handleInputChange} className="p-2 w-full  h-full outline-none  py-3" placeholder="Filtru Descriere" />
                  </th>
                    ...
                    <div className=" flex  justify-evenly items-center">
                      ...
                      <div className="flex justify-center  items-center">
                        ...
                        >
                          ...
                          <FontAwesomeIcon className="text-white" icon={ascendentTime == null ? faSort : ascendentTime == true ? faSortDown : faSortUp} />
                        </div>
                      </div>
                    </div>
                  </th>
