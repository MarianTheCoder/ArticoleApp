...
import { faAddressCard, faCalendar, faLocationDot, faUser, faPlus, faTrash, faImages, faCamera, faPen, faPenToSquare, faArrowRight, faXmark } from "@fortawesome/free-solid-svg-icons";
...
export default function PlanPinDrawer({ open, pin, onClose, remainCommentState, setRemainCommentState, onEditPin, onDeletePin }) {
  ...
  // pin photos
  const photos = useMemo(() => [localPin?.photo1_path, localPin?.photo2_path, localPin?.photo3_path].filter(Boolean), [localPin]);
  // comments
  ...
  }, [savingRecord, recDesc, recPhotos, recChangeStatus, recStatus, remainCommentState, localPin?.id, localPin?.status, user?.id, resetPanel]);
  ...
  const submitEdit = useCallback(async () => {
    ...
    const photosChanged = editNewPhotos.length > 0 || !arraysEqual(editExistingPhotos, editInitialPhotos);
  ...
  }, [isEditing, editSaving, editingCommentId, editDesc, editInitialDesc, editExistingPhotos, editInitialPhotos, editNewPhotos, user?.id, maxPhotos, localPin?.id, localPin?.plan_id]);
  ...
  const handleDropUpload = (e) => {
    ...
    const currentCount = isEditing ? editExistingPhotos.length + editNewPhotos.length : recPhotos.length;
  ...
  const recPhotoUrls = useMemo(() => recPhotos.map((f) => URL.createObjectURL(f)), [recPhotos]);
  useEffect(() => {
    return () => {
      recPhotoUrls.forEach((u) => URL.revokeObjectURL(u));
    };
  }, [recPhotoUrls]);

  const editNewPhotoUrls = useMemo(() => editNewPhotos.map((f) => URL.createObjectURL(f)), [editNewPhotos]);
  useEffect(() => {
    return () => {
      editNewPhotoUrls.forEach((u) => URL.revokeObjectURL(u));
    };
  }, [editNewPhotoUrls]);
  ...
  return (
    <div className="absolute inset-0 z-40 text-base pointer-events-none overflow-hidden">
      {/* Backdrop */}
      <div className={["absolute inset-0 bg-black/40  transition-opacity duration-300", open ? "opacity-100 pointer-events-auto" : "opacity-0"].join(" ")} onClick={onClose} aria-hidden />
        ...
        <div className="flex-1 overflow-y-auto p-5 text-foreground relative">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <Button variant="destructive" size="icon" className="rounded-xl h-10 w-10 shadow-sm" onClick={onClose} aria-label="Închide" title="Închide">
                <FontAwesomeIcon icon={faXmark} className="text-lg" />
              </Button>

              <Separator orientation="vertical" className="h-8 mx-1" />
              <Button className="bg-green-600 hover:bg-green-700 text-white rounded-xl h-10 w-10 shadow-sm" size="icon" onClick={() => onEditPin(localPin)} aria-label="Editează pin" title="Editează pin">
                <FontAwesomeIcon icon={faPen} />
              </Button>

              <Button variant="destructive" size="icon" className="rounded-xl h-10 w-10 shadow-sm" onClick={() => onDeletePin(localPin)} aria-label="Șterge pin" title="Șterge pin">
          ...
          <div className="mb-1 text-2xl font-extrabold tracking-tight">{localPin?.title || "Pin nou"}</div>
          <div className="mb-3 text-muted-foreground leading-relaxed">{localPin?.description || "Fără descriere"}</div>
          ...
          {photos.length > 0 && (
            <div className="mb-4 grid grid-cols-3 gap-2">
              {photos.map((uri, idx) => (
                ...
                >
                  <img src={`${baseURL}${uri}`} alt={`pin-photo-${idx}`} className="w-full h-40 object-cover group-hover:scale-[1.02] transition-transform duration-200" loading="lazy" />
                </button>
              ))}
            </div>
          )}
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="h-9 w-9 rounded-full bg-background border border-input grid place-items-center text-sm font-semibold text-secondary-foreground">{(c.user_name?.[0] || "—").toUpperCase()}</div>
                          <div className="truncate">
                            <div className="font-semibold text-foreground truncate">{c.user_name || "—"}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button className="bg-green-600 hover:bg-green-700 h-8 w-8 text-white rounded-xlshadow-sm" size="icon" onClick={() => openEdit(c)} disabled={panelOpen}>
                            <FontAwesomeIcon icon={faPen} />
                          </Button>
                        </div>
                      </div>
                      ...
                      {c.status_from && c.status_to && (
                        <div className="flex items-center gap-2 mt-3 mb-2 flex-wrap">
                          <span className="text-sm uppercase tracking-wide text-muted-foreground font-semibold">Status</span>
                          <span className="font-semibold" style={{ color: STATUS_COLORS[c.status_from] || "inherit" }}>
                            {STATUS_LABELS[c.status_from] || c.status_from}
                          </span>
                          <span className="opacity-60 text-muted-foreground">
                            <FontAwesomeIcon icon={faArrowRight} />
                          </span>
                          <span className="font-semibold" style={{ color: STATUS_COLORS[c.status_to] || "inherit" }}>
                      ...
                      {/* Body */}
                      <div className="text-foreground leading-relaxed mt-2">{c.body_text ? <p className="whitespace-pre-wrap text-sm">{c.body_text}</p> : <span className="text-muted-foreground italic text-sm">Fără descriere</span>}</div>
                          ...
                          <div className="flex gap-2 flex-wrap">
                            {c.photos.slice(0, 3).map((p, pi) => (
                              <button
                                ...
                                title={`Foto ${pi + 1}`}
                              >
                                <img src={`${baseURL}${p}`} className="w-full h-full object-cover group-hover:scale-[1.05] transition-transform duration-200" alt="comment-photo" loading="lazy" />
                              </button>
                            ))}
                          </div>
                        </>
          ...
          {lbOpen && <Lightbox open close={() => setLbOpen(false)} index={lbIndex} slides={slides} plugins={[Zoom, Thumbnails]} controller={{ closeOnBackdropClick: true }} />}
          {lbOpenCom && <Lightbox open close={() => setLbOpenCom(false)} index={lbIndexCom} slides={lbComSlides} plugins={[Zoom, Thumbnails]} controller={{ closeOnBackdropClick: true }} />}
        </div>
            ...
            <Card className="w-full rounded-2xl shadow-md border-input">
              <div className="p-5 flex flex-col gap-5">
                ...
                <div className="text-xl flex items-center justify-between font-extrabold text-foreground">
                  ...
                  {!isEditing && (
                    <div className="flex items-center space-x-2">
                      <Checkbox id="remain-checkbox" checked={remainCommentState} onCheckedChange={(checked) => setRemainCommentState(checked)} className="h-6 w-6" />
                    ...
                    return (
                      <div
                        ...
                        className={[
                          ...
                          isFull ? "opacity-60 cursor-not-allowed bg-muted" : "cursor-pointer hover:bg-background/10",
                          isDragOverUpload && !isFull ? "border-primary bg-primary/5" : "border-input",
                        ].join(" ")}
                      >
                        <div className="inline-flex items-center gap-2 text-foreground">
                          <FontAwesomeIcon icon={faImages} />
                          <span className="font-medium text-sm">{isFull ? "Limită fotografii atinsă" : "Click sau trage din Galerie"}</span>

                          <input ref={uploadInputRef} type="file" accept="image/*" multiple hidden onChange={isEditing ? onPickEditPhotos : onPickRecPhotos} disabled={isFull} />
                        </div>
                      </div>
                    );
                  })()}
                        ...
                        <div className="flex gap-2 pb-1">
                          {editExistingPhotos.map((p, i) => (
                            <div key={`ex-${i}`} className="relative flex-shrink-0">
                              <Button size="icon" className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-destructive text-xs shadow z-10" onClick={() => removeExistingEditPhotoAt(i)}>
                                ✕
                              </Button>
                          ...
                          {editNewPhotos.map((f, i) => (
                            <div key={`new-${i}`} className="relative flex-shrink-0">
                              <Button variant="destructive" size="icon" className="absolute -top-2 -right-2 w-6 h-6 rounded-full text-xs shadow z-10" onClick={() => removeNewEditPhotoAt(i)} title="Șterge">
                                ✕
                              </Button>
                              <div className="relative rounded-xl border border-input overflow-hidden shadow-sm w-24 h-24">
                        ...
                        <div className="flex gap-2 pb-1">
                          {recPhotos.map((f, i) => (
                            <div key={`rec-${i}`} className="relative flex-shrink-0">
                              <Button variant="destructive" size="icon" className="absolute -top-2 -right-2 w-6 h-6 rounded-full text-xs shadow z-10" onClick={() => removeRecPhotoAt(i)} title="Șterge">
                                ✕
                              </Button>
                              <div className="relative rounded-xl border border-input overflow-hidden shadow-sm w-24 h-24">
                ...
                <div>
                  ...
                  <Textarea rows={3} value={isEditing ? editDesc : recDesc} onChange={(e) => (isEditing ? setEditDesc(e.target.value) : setRecDesc(e.target.value))} placeholder="Adaugă detalii relevante…" />
                </div>
                ...
                {!isEditing && (
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Checkbox id="status-toggle" checked={recChangeStatus} onCheckedChange={setRecChangeStatus} className="h-6 w-6" />
                    </div>

                    {recChangeStatus && (
                      <Select value={recStatus} onValueChange={(value) => setRecStatus(value)}>
                ...
                <div className="flex gap-3 pt-2">
                  <Button variant="destructive" className="flex-1 rounded-full" onClick={cancelPanel} disabled={savingRecord}>
                    Anulează
                  </Button>

                  <Button className="flex-1 rounded-full bg-blue-600 hover:bg-blue-700 text-white" onClick={isEditing ? submitEdit : submitRecord} disabled={isEditing ? editSaving : savingRecord}>
                    {isEditing ? (editSaving ? "Se actualizează…" : "Actualizează") : savingRecord ? "Se salvează…" : "Salvează"}
                  </Button>
                </div>
              </div>
            </Card>
          ) : (
            <Button className="rounded-full flex-1 w-full gap-2 text-base h-12" onClick={togglePanel} title="Adaugă înregistrare">
