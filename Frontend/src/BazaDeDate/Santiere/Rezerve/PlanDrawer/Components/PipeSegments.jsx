...
export default function PipeSegments({ pipeSegments }) {
  return pipeSegments.map((seg, idx) => {
    const segmentPoints = [seg.start.x, seg.start.y, seg.end.x, seg.end.y];
    ...
    return (
      <React.Fragment key={`seg-${idx}`}>
        <Line points={segmentPoints} stroke={outerColor} strokeWidth={width + 2 * edgeWidth} />
        <Line points={segmentPoints} stroke={innerColor} strokeWidth={width} />
        <Line points={segmentPoints} stroke={centerColor} strokeWidth={width / 6} dash={[20, 12]} />
      </React.Fragment>
    );
  });
}
