...
export default function PlanDrawer({ plan }) {
  ...
  const handleWheel = useCallback((e) => {
    ...
    const vpPoint = viewer.viewport.pointFromPixel(new OpenSeadragon.Point(pos.x, pos.y), true);
  ...
  const handleMouseMove = useCallback(
    (e) => {
      ...
      const lastPoint = pipeSegments.length ? pipeSegments[pipeSegments.length - 1].end : null;
  ...
  const handleStageClick = useCallback(
    (e) => {
      ...
      setPipeSegments((prevSegs) => {
        const { nextSegs, newElbow, newReducer } = computePipeSegmentsForClick(prevSegs, imgRaw, shift, STEP_DEG, PIPE_WIDTH, PIPE_COLOR, COT_VISUAL_FACTOR, MIN_SEGMENT_LEN);
        ...
        {size.w > 0 && size.h > 0 && (
          ...
          >
            <Layer listening>
              ...
              <PreviewHover hoverPoint={hoverPoint} pipeSegments={pipeSegments} PIPE_WIDTH={PIPE_WIDTH} PIPE_COLOR={PIPE_COLOR} PIPE_EDGE={PIPE_EDGE} CENTER_WIDTH={CENTER_WIDTH} />
