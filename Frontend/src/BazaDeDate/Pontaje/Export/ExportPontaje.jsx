...
// Add this helper at the top of the file (module level)
...
export default async function ExportPontaje({ selectedUserIds, dates, selectedCompany, includeRapoarte }) {
  try {
    ...
    const companyName = data?.companie?.nume ?? "Baly Energies SAS";
    ...
    const headerRow1 = [
      ...
      ...weekGroups.flatMap((g) => [{ text: g.label, style: "th", alignment: "center", colSpan: g.span }, ...Array(g.span - 1).fill("")]),
      { text: "TOTAL", style: "th", alignment: "center", rowSpan: 2 },
      ...(hasVisible ? [{ text: "DIN CARE:", style: "th", alignment: "center", colSpan: visibleSites.length }, ...Array(Math.max(0, visibleSites.length - 1)).fill("")] : []), // nothing if no visible sites
    ];
      ...
      footer: function (currentPage, pageCount) {
        return {
          ...
          table: {
            ...
            body: [
              [
                // In footer, replace the first cell with:
                {
                  table: {
                    widths: [80],
                    heights: [30],
    ...
    toast.error("Eroare la generarea pontajelor. Te rugăm să încerci din nou sau să contactezi suportul.");
  }
}
