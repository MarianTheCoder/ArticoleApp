...
const { folderImage, logo, userImage, materialeImage, utilajeImage, transportImage } = require("../jobs/base64Items"); // <- adjust path if base64Items lives elsewhere
...
async function getWeekAggByUserPerDay(range, userIds = null) {
  ...
  let sql = `
    SELECT
      ...
      COALESCE(s.nume, 'Fără șantier') AS santier_name,
    ...
    WHERE sl.session_date >= ?
      ...
      /*USER_FILTER*/
    GROUP BY sl.user_id, sl.session_date, s.id, s.nume, s.culoare_hex
...
function buildOneUserWeeklyTable(userName, daysPayload, { pageBreakAfter = false } = {}, currentIdx = 0) {
  ...
  return [{ text: userName, style: "userTitle", margin: [0, 10, 0, 4] }, tableNode];
}
