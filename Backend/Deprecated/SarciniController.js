const getReteteFromOfertaForSarcina = async (req, res) => {
  ...
  try {
    ...
    // 2) Reper info
    const [[reperRow]] = await global.db.execute(`SELECT reper1, reper2 FROM oferta_parts WHERE id = ?`, [id]);
    ...
    console.log("Error getting retete with prices:", err);
...
const getMaterialeForSelectedRetete = async (req, res) => {
  ...
  try {
    ...
    console.log("materialsForSelection error:", err);
    res.status(500).json({ error: "Database error" });
  } finally {
    connection.release();
  }
