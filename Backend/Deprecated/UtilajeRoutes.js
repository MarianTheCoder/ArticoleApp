...
router.post("/api/setUtilajDef", authenticateToken("utilaje", "c"), upload.single("poza"), async (req, res) => {
  ...
  try {
    const { limba, clasa_utilaj, cod_definitie, utilaj, utilaj_fr, descriere, descriere_fr, unitate_masura, cost_amortizare, pret_utilaj, childs = null } = req.body;
    ...
    const [result] = await conn.execute(sql, [limba, clasa_utilaj, cod_definitie, utilaj, utilaj_fr, descriere, descriere_fr, photoPath, unitate_masura, cost_amortizare, pret_utilaj]);

    const newDefinitionId = result.insertId;

    // 🧬 Dacă ai trimis childs, clonează și `Utilaje` copil
    if (childs) {
      const [rows] = await conn.query(`SELECT * FROM Utilaje WHERE definitie_id = ?`, [childs]);
        ...
        if (newPhotoPath && !newPhotoPath.includes("no-image-icon.png")) {
          ...
          try {
            ...
            try {
              ...
              console.log("❌ Fallback eșuat la clonarea imaginii:", fallbackErr);
      ...
      photoUrl, status_utilaj, cantitate, cost_amortizare, pret_utilaj
          ...
          [newDefinitionId, row.cod_utilaj, row.furnizor, row.descriere, row.descriere_fr, newPhotoPath, row.status_utilaj, row.cantitate, row.cost_amortizare, row.pret_utilaj],
        );
      }
    }
...
router.post("/api/setUtilaj", authenticateToken("utilaje", "c"), upload.single("poza"), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      const [parentRows] = await conn.execute(`SELECT photoUrl FROM Utilaje_Definition WHERE id = ? LIMIT 1`, [id]);
...
router.get("/api/utilajeDef", async (req, res) => {
  try {
    const { offset = 0, limit = 10, cod = "", utilaj = "", descriere = "", limba = "", clasa_utilaj = "" } = req.query;
...
router.delete("/api/deleteUtilajDef/:id", authenticateToken("utilaje", "s"), async (req, res) => {
  ...
  try {
    ...
    // 🧽 1. Pozele din copii
    const [copiiRows] = await conn.execute(`SELECT photoUrl FROM Utilaje WHERE definitie_id = ?`, [id]);

    // 🧽 2. Poza din definiție
    const [defRows] = await conn.execute(`SELECT photoUrl FROM Utilaje_Definition WHERE id = ?`, [id]);
    ...
    // 🧨 4. Șterge definiția
    const [deleteDef] = await conn.execute(`DELETE FROM Utilaje_Definition WHERE id = ?`, [id]);
    ...
    res.status(200).json({
      message: "Definiția și toate instanțele de utilaje au fost șterse cu succes!",
    });
  } catch (err) {
...
router.delete("/api/utilaj/:id", authenticateToken("utilaje", "s"), async (req, res) => {
  ...
  try {
    ...
    const [result] = await global.db.execute(`DELETE FROM Utilaje WHERE id = ?`, [id]);

    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "Utilaj inexistent sau deja șters." });
    }
...
router.put("/api/editUtilajDef/:id", authenticateToken("utilaje", "e"), upload.single("poza"), async (req, res) => {
  const { id } = req.params;
  const { limba, clasa_utilaj, cod_definitie, utilaj, utilaj_fr, descriere, descriere_fr, unitate_masura, cost_amortizare, pret_utilaj } = req.body;
  ...
  try {
    ...
    const [rows] = await global.db.execute(`SELECT photoUrl FROM Utilaje_Definition WHERE id = ?`, [id]);

    if (rows.length === 0) {
      return res.status(404).json({ message: "Definiția utilajului nu a fost găsită." });
    }
    ...
    res.status(200).json({ message: "Definiție utilaj actualizată cu succes!" });
  } catch (error) {
    console.log("Eroare server:", error);
...
router.put("/api/editUtilaj", authenticateToken("utilaje", "e"), upload.single("poza"), async (req, res) => {
  const { id, definitie_id, cod_utilaj, furnizor, descriere, descriere_fr, cantitate, cost_amortizare, pret_utilaj, status_utilaj } = req.body;
  console.log("Received data:", req.body);
  try {
    if (!id || isNaN(id)) {
      return res.status(400).json({ message: "ID invalid sau lipsă." });
    ...
    const [result] = await global.db.execute(updateQuery, [definitie_id, cod_utilaj, furnizor, descriere, descriere_fr, cantitate, cost_amortizare, pret_utilaj, status_utilaj, newPhotoPath, id]);

    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "Utilajul nu a fost actualizat." });
    }
    ...
    console.log("Eroare la update Utilaj:", error);
    res.status(500).json({ message: "Eroare internă la actualizare." });
  }
});
