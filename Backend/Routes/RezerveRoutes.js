...
router.post("/plans/:lucrareId/upload", authenticateToken("rezerve", "c"), upload.single("planPdf"), async (req, res) => {
  ...
  try {
    ...
    await execFileAsync("pdftoppm", ["-png", "-singlefile", "-f", "1", "-l", "1", "-r", String(density), pdfAbs, outBase]);
    ...
    await sharp(imgAbs).resize({ width: midW, withoutEnlargement: true }).png().toFile(imgAbsMid);

    await sharp(imgAbs).resize({ width: lowW, withoutEnlargement: true }).png().toFile(imgAbsLow);
    ...
    await sharp(imgAbs).png().tile({ size: TILE, overlap: 0, layout: "dz" }).toFile(`${tilesOutBase}.dzi`);
...
router.post("/lucrari3d", authenticateToken("rezerve", "c"), upload3D.single("modelFile"), async (req, res) => {
  try {
    ...
    // 1) Get șantier name
    const [sRows] = await global.db.execute(`SELECT id, nume FROM S01_Santiere WHERE id = ? LIMIT 1`, [Number(santier_id)]);
    ...
    const [result] = await global.db.execute(
      `INSERT INTO S08_Rezerve_Lucrari
                ...
                VALUES (?,?,?,?,?)
            `,
      [Number(santier_id), name.trim(), description?.trim() || null, 1, publicModelPath],
    );
    ...
    console.log("lucrari3d error:", err);
...
router.post("/plans/plansPreview", authenticateToken("rezerve", "c"), upload.single("pdf"), async (req, res) => {
  ...
  try {
    ...
    await execFileAsync("pdftoppm", ["-png", "-singlefile", "-f", "1", "-l", "1", "-r", String(previewDpi), pdfAbs, outBase]);
    const imgAbs = `${outBase}.png`;
    ...
    console.log("plansPreview error:", err);
    return res.status(500).json({ error: err.message || "Preview failed" });
  }
});

const toAbsFromPublic = (publicPath) => path.join(__dirname, "..", publicPath.replace(/^\/+/, ""));
...
router.post("/plans/commitNewPlan", authenticateToken("rezerve", "c"), async (req, res) => {
  ...
  try {
    ...
    await execFileAsync("pdftoppm", ["-png", "-singlefile", "-f", "1", "-l", "1", "-r", String(targetDpi), pdfAbs, imgNameBase]);
    ...
    // 4. Build DZI tiles
    const TILE = 256;
    await sharp(imgAbs).png().tile({ size: TILE, overlap: 0, layout: "dz" }).toFile(dziAbs);
    ...
    console.log("plans/commit error:", err);
