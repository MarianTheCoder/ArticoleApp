  ...
  try {
    ...
    const [plans] = await global.db.execute(
      `SELECT 
                ...
                pdf_path, image_path, thumb_path,
...
const lucrariDELETE = async (req, res) => {
  ...
  try {
    ...
    const [result] = await conn.execute(`DELETE FROM S08_Rezerve_Lucrari WHERE id = ?`, [id]);
    if (result.affectedRows === 0) {
...
const plansDELETE = async (req, res) => {
  try {
    ...
    const [result] = await global.db.execute(`DELETE FROM S08_Rezerve_Plans WHERE id = ?`, [id]);
    if (result.affectedRows === 0) {
...
const pinsPOST = async (req, res) => {
  try {
    const { plan_id, title, description, x_pct, y_pct, status = "new", priority = "medium", user_id, assigned_user_id = null, due_date_utc = null, reper = null } = req.body;
    ...
    const [result] = await global.db.execute(
      ...
      [user_id, plan_id, codeStr, title || null, description || null, reper || null, x_pct, y_pct, status, priority, assigned_user_id, due_date_utc || null],
    );
    ...
    const [tokensRows] = await global.db.execute(
      `SELECT upt.token
                ...
                JOIN S01_Atribuire_Activitate su ON su.user_id = upt.user_id  
                WHERE su.santier_id = ?
    ...
    if (tokens.length > 0) {
      const message = {
        notification: {
          ...
          body:
            `${pin.user_name || "Un utilizator"} a adăugat pinul #${pin.code}: ${pin.title || ""}\n` +
...
const comentariiPOST = async (req, res) => {
  try {
    ...
    const [[pin]] = await global.db.execute(`SELECT id, plan_id, code, status FROM S09_Rezerve_Pins WHERE id = ? LIMIT 1`, [pin_id]);
    ...
    if (willChange) {
      await global.db.execute(`UPDATE S09_Rezerve_Pins SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [status_to, pin_id]);
    } else {
      await global.db.execute(`UPDATE S09_Rezerve_Pins SET  updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [pin_id]);
    }
    await global.db.execute(
      `INSERT INTO S09_Rezerve_Pin_Seen (user_id, pin_id, last_seen_at)
                VALUES (?, ?, CURRENT_TIMESTAMP)
    ...
    try {
      ...
      if (santierId) {
        ...
        if (tokens.length > 0) {
          ...
          const statusPart = comment.status_from && comment.status_to ? ` • Status: ${comment.status_from} → ${comment.status_to}` : "";
...
const exportPDF = async (req, res) => {
  try {
    ...
    const [[santierDetalii]] = await conn.execute(`SELECT * FROM  Santiere_detalii where santier_id = ? LIMIT 1`, [santier.santier_id]);
    ...
    for (const c of comRows) {
      ...
      arr.push({
        ...
        photos: [c.photo1_path, c.photo2_path, c.photo3_path].filter(Boolean),
      });
    ...
    const pins = pinRows.map((p) => ({
      ...
      photos: [p.photo1_path, p.photo2_path, p.photo3_path].filter(Boolean),
...
const markPlanSeenIndividual = async (req, res) => {
  ...
  if (!planId || !pinId || !userId) return res.status(400).json({ error: "bad params" });
...
const saveZonesPOST = async (req, res) => {
  ...
  try {
    const { plan_id, user_id, zone_title, zone_description, zones = [] } = req.body || {};
    ...
    // 2) Create pattern
    const name = zone_title?.trim() || `Pattern ${new Date().toISOString().slice(0, 10)}`;
    ...
    const toPct = (x, axis) => (axis === "x" ? x / width_px : x / height_px);
    ...
    for (const z of zones) {
      ...
      const ptsPct = (z.points || []).map((v, i) => (i % 2 === 0 ? toPct(v, "x") : toPct(v, "y")));
   ...
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, // ⬅️ fără CAST
    ...
    await conn.query(`UPDATE S08_Rezerve_Plans SET pattern_id = ? WHERE id = ?`, [patternId, plan_id]);
...
const pinsEditPOST = async (req, res) => {
  ...
  try {
    ...
    let deleteExisting = [];
    try {
      deleteExisting = Array.isArray(delRaw) ? delRaw : JSON.parse(delRaw || "[]");
    } catch (_) {}
    ...
    // 4) Deletions by slot
    const deletions = deleteExisting.map(String).filter((s) => ["photo1", "photo2", "photo3"].includes(s));

    for (const slot of deletions) {
      if (slot === "photo1" && photo1) {
        await tryUnlink(photo1);
        photo1 = null;
      }
      if (slot === "photo2" && photo2) {
        await tryUnlink(photo2);
        photo2 = null;
      }
      if (slot === "photo3" && photo3) {
        await tryUnlink(photo3);
        photo3 = null;
      }
    }
    ...
    fields.push("photo1_path = ?");
    params.push(photo1);
    fields.push("photo2_path = ?");
    params.push(photo2);
    fields.push("photo3_path = ?");
    params.push(photo3);
...
const deletePin = async (req, res) => {
  ...
  try {
    ...
    const [[pinRow]] = await global.db.execute(`SELECT id, photo1_path, photo2_path, photo3_path FROM S09_Rezerve_Pins WHERE id = ? LIMIT 1`, [pinId]);
    if (!pinRow) return res.status(404).json({ error: "Pin not found" });

    const [commentRows] = await global.db.execute(`SELECT photo1_path, photo2_path, photo3_path FROM S09_Rezerve_Pin_Comments WHERE pin_id = ?`, [pinId]);

    const filePaths = [];
    // pin photos
    [pinRow.photo1_path, pinRow.photo2_path, pinRow.photo3_path].filter(Boolean).forEach((p) => filePaths.push(p));
    // comment photos
    for (const c of commentRows) {
      [c.photo1_path, c.photo2_path, c.photo3_path].filter(Boolean).forEach((p) => filePaths.push(p));
    }
    ...
    await conn.execute(`DELETE FROM S09_Rezerve_Pin_Comments WHERE pin_id = ?`, [pinId]);

    // 4) Delete pin seen
    await conn.execute(`DELETE FROM S09_Rezerve_Pin_Seen WHERE pin_id = ?`, [pinId]);

    // 5) Delete the pin
    const [delPinRes] = await conn.execute(`DELETE FROM S09_Rezerve_Pins WHERE id = ?`, [pinId]);
...
const comentariiEDIT = async (req, res) => {
  ...
  try {
    ...
    const hasKeepPhotosKey =
      Object.prototype.hasOwnProperty.call(req.body, "keep_photos[]") || Object.prototype.hasOwnProperty.call(req.body, "keep_photos") || Object.prototype.hasOwnProperty.call(req.body, "keepPhotos");

    // Normalize to array (allow single string)
    const keepRaw = req.body["keep_photos[]"] ?? req.body.keep_photos ?? req.body.keepPhotos ?? null;
    const keep_photos = Array.isArray(keepRaw) ? keepRaw.filter(Boolean) : keepRaw ? [keepRaw] : [];
    ...
    const [[pin]] = await global.db.execute(`SELECT id, plan_id, code, status FROM S09_Rezerve_Pins WHERE id = ? LIMIT 1`, [comment.pin_id]);
    if (!pin) return res.status(404).json({ error: "Pin not found" });
    ...
    const toDelete = [photo1, photo2, photo3].filter(Boolean).filter((p) => !effectiveKeep.has(p));
    ...
    await global.db.execute(`UPDATE S09_Rezerve_Pins SET updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [pin.id]);
    if (user_id) {
      await global.db.execute(
        `INSERT INTO S09_Rezerve_Pin_Seen (user_id, pin_id, last_seen_at)
              VALUES (?, ?, CURRENT_TIMESTAMP)
